...
The exact actions available to you depend on your role, permissions, partner configuration, and the community state.
...
Creating a subscription also requires Self-Service to be enabled for your partner.
...
Sekoia enables subscription creation manually for eligible partners after the applicable commercial and legal checks are complete. The **Add subscription** entry point remains available to users who can manage subscriptions, but its options depend on the partner configuration.
