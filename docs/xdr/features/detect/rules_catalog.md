...
- **Sekoia Verified Rules**: rules with the valid icon are verified ones. These rules are created for you by Sekoia.io's Threat & Detection Research team and already built-in. Verified rules are constantly updated to improve detection. Furthermore, they follow a specific process to test them and be certain they won't cause many false positives. This process is described in our blogpost [XDR detection engineering at scale: crafting detection rules for SecOps efficiency](https://blog.sekoia.io/xdr-detection-rules-at-scale/). This set of more than 1000+ rules can be used to detect known threats, attack patterns, etc.
- **Your Rules (Custom)**: rules created by your team that are specific to your organization.
- **Integration Rules (Verified)**: detection rules specifically designed to monitor and alert on suspicious activity from third-party security tools and platforms integrated with Sekoia.io. They are maintained by the Integrations team, clearly distinguished with partner branding in the Rules Catalog, and help users quickly identify and filter detections originating from external sources.
...
- **Active**: The rule is currently maintained and actively detecting threats.
    - Continue to receive updates and improvements from rule maintainers
    - Generate alerts as configured in your detection environment
    - Are counted in your organization's verified rule coverage metrics
- **Scheduled for Deprecation**: Rule owners have flagged a rule for future retirement due to obsolescence, replacement, or maintenance considerations. When a rule enters this state:
    - A "Valid until" date is displayed on the rule card and detailed view, indicating when the rule will transition to Deprecated status
    - A warning tag appears on the rule to alert users of the impending change
    - Users subscribed to deprecation notifications receive an alert about the scheduled change
    - The rule continues to function and generate alerts until the validity date is reached
    - Rule owners need to provide a deprecation reason explaining why the rule is being retired (e.g., "Replaced by improved detection logic," "Integration no longer supported," "Outdated threat signature")
...
- **Deprecated**: The validity date has been reached and the rule is no longer actively maintained. When a rule reaches Deprecated status:
  
    - A prominent warning banner is displayed on the rule card and detailed view
    - The rule remains enabled in your environment unless you choose otherwise
    - Users subscribed to deprecation notifications receive a final notification
    - The rule continues to generate alerts until you manually disable it or you choose to automatically disable all deprecated rules in the Catalog. 
