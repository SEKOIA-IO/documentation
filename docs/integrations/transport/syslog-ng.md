...
The first thing to do is to add certification authority (CA) files in the `/etc/syslog-ng/ca.d` directory. To proceed, you can execute the following commands in your favorite shell:

<details><summary>Click here to display commands</summary>

```bash
# cat > /etc/syslog-ng/ca.d/2e5ac55d.0 << EOF
-----BEGIN CERTIFICATE-----
...
# cat > /etc/syslog-ng/ca.d/4f06f81d.0 << EOF
-----BEGIN CERTIFICATE-----
