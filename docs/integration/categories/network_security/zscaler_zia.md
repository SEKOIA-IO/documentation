...
9. For `Feed Output Format`, paste the following format according to the log type:
	...
	=== "SaaS Security Activity"
		...
		```
		<134>1 %d{yyyy}-%02d{mm}-%02d{dd}T%02d{hh}:%02d{mm}:%02d{ss}Z zscaler-nss NSS - - - \{"sourcetype":"zscalernss-casb","event":\{"login":"%s{username}","tenant":"%s{tenant}","object_type":"%d{objtype1}","applicationname":"%s{appname}","object_name_1":"%s{objnames1}","object_name_2":"%s{objnames2}"\}\}
		```
