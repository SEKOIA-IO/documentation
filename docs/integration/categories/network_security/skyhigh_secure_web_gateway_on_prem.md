...
title: McAfee Web Gateway / Skyhigh Secure Web Gateway - On Prem
...
- Add the following line:
  ...
  ```
  Replace the `<log concentrator ip>` placeholder with the ip address of our log concentrator and `<port>` by the listening port on the [Syslog Forwarding](/integration/ingestion_methods/syslog/sekoiaio_forwarder.md) for the Skyhigh raw events.
