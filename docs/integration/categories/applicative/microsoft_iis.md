...
!!! warning "Important prerequisite regarding timezones"
    Log files generated by Microsoft IIS do not contain timezone information. To avoid time offset issues when events are parsed by Sekoia.io (e.g. events appearing to happen in the future), you *must* configure your Windows server's timezone to **UTC**.
    
    You can easily set the system timezone to UTC using the following PowerShell command as Administrator:
    ```powershell
