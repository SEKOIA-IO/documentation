...
!!! warning "Important prerequisite regarding timezones"
    Log files generated by F5 BIG-IP do not contain timezone information. To avoid time offset issues when events are parsed by Sekoia.io (e.g. events appearing to happen in the future), you *must* configure your F5 BIG-IP system's timezone to **UTC**.
