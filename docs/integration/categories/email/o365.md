...
- Copilot and General audit events not included in the other log categories (`Audit.General`)
...
- Have auditing turned on for your organization (see [Turn auditing on or off](https://docs.microsoft.com/en-us/microsoft-365/compliance/turn-audit-log-search-on-or-off))
...
If you receive a permission error (HTTP 401) in the log, check if policies were defined in the Data Loss Prevention section in [Microsoft Purview](https://purview.microsoft.com/datalossprevention).
If yes, add the 'ActivityFeed.ReadDlp' permission to your application and grant tenant admin consent for that permission.
