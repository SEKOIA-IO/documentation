...
uuid: f7da3627-6016-415d-9885-231e1455a7a5 name: Harfanglab EDR type: asset
...
!!! warning - Ensure the user account exists before generating an API token. - The API token is displayed only once at creation. Save it securely; if you lose it, you must generate a new one. - When copying the API key, ensure you copy only the raw key value (excluding any `token_` prefix if displayed).
...
!!! warning - Make sure to include your port number in the `URL` field by appending it to the URL, for example: `https://your-harfanglab-instance.com:8443`.

{!\_shared\_content/operations\_center/integrations/generated\_assets\_documentation/harfanglab\_device.md!}
