...
uuid: 226573e7-3969-4e31-bb3a-0846760b35b9 name: Microsoft Active Directory User type: asset
...
1.  `Service Account`: A dedicated service account with read-only permissions.
    ...
    !!! tip "Recommended: Create a least-privilege account" Instead of using an admin account, create a dedicated service account with minimal permissions:
...
!!! Warning Ensure that the account used has a password that does not expire, or remember to update the connector settings in Sekoia.io whenever the password is changed, otherwise the synchronization will fail.
...
{!\_shared\_content/operations\_center/integrations/generated\_assets\_documentation/microsoft-ad\_user.md!}
