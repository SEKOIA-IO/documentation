import { filter_falsey_keys } from "../utils";
    ...
    for (const p in s.paths || {}) {
      for (const method in s.paths[p]) {
        if (method !== "parameters" && !s.paths[p][method].tags?.length) {
          console.warn(`Endpoint ${method} ${p} has no tag, skipping`, s);
          continue;
        }
        if (paths[p]?.[method]) {
            ...
            Object.values(p).forEach((endpoint) => {
              ...
              if (bodyParams?.length > 0) {
                ...
                for (const p of bodyParams) {
                  Object.entries(p.schema?.properties || {}).forEach(
                    ([k, v]) => (schema.properties[k] = v)
                  );
                }
