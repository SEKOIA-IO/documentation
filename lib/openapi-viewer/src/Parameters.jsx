...
export const Parameters = {
    ...
    setup(props) {
        return () => {
            ...
            if (bodySchema?.properties) bodyParameters.push(...Object.entries(bodySchema.properties).map(([name, p]) => ({ ...p, name })))
...
export const Property = (prop, override = {}, stack = null, level = 0) => {
    ...
    >
        ...
        <td class="type">{type || "UNKNOWN!!"} {!!p.format && <>({p.format})</>}
        </td>
        ...
        {prop.expanded && Object.entries(subProperties || {}).map(([name, p]) => Property(p, { name }, null, level + 1))}
    </>
}
