
### Event Categories


The following table lists the data source offered by this integration.

| Data Source | Description                          |
| ----------- | ------------------------------------ |
| `DNS records` | NIOS provides detailed logs on handled DNS queries |





In details, the following table denotes the type of events produced by this integration.

| Name | Values |
| ---- | ------ |
| Kind | `` |
| Category | `authentication`, `file`, `network` |
| Type | `deletion`, `end`, `info`, `protocol`, `start` |




### Transformed Events Samples after Ingestion

This section demonstrates how the raw logs will be transformed by our parsers. It shows the extracted fields that will be available for use in the [built-in detection rules](/xdr/features/detect/rules_catalog.md) and hunting activities in the [events page](/xdr/features/investigate/events.md). Understanding these transformations is essential for analysts to create effective detection mechanisms with [custom detection rules](/xdr/features/detect/sigma.md) and to leverage the full potential of the collected data.

=== "query_log.json"

    ```json
	
    {
        "message": "client @0x7f0000000000 192.0.2.70#55575 (app.example.test): query: app.example.test IN AAAA + (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "app.example.test",
                "subdomain": "app.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "app.example.test"
            ],
            "ip": [
                "192.0.2.70"
            ]
        },
        "source": {
            "address": "192.0.2.70",
            "ip": "192.0.2.70",
            "port": 55575
        }
    }
    	
	```


=== "query_log_category_prefix.json"

    ```json
	
    {
        "message": "queries: client @0x7f0000000000 192.0.2.70#55575 (app.example.test): query: app.example.test IN AAAA + (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "app.example.test",
                "subdomain": "app.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "infoblox": {
            "ddi": {
                "category": "queries"
            }
        },
        "related": {
            "hosts": [
                "app.example.test"
            ],
            "ip": [
                "192.0.2.70"
            ]
        },
        "source": {
            "address": "192.0.2.70",
            "ip": "192.0.2.70",
            "port": 55575
        }
    }
    	
	```


=== "query_log_cd.json"

    ```json
	
    {
        "message": "client 192.0.2.70#55575 (app.example.test): query: app.example.test IN AAAA +TC (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "CD",
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "app.example.test",
                "subdomain": "app.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "network": {
            "transport": "tcp"
        },
        "related": {
            "hosts": [
                "app.example.test"
            ],
            "ip": [
                "192.0.2.70"
            ]
        },
        "source": {
            "address": "192.0.2.70",
            "ip": "192.0.2.70",
            "port": 55575
        }
    }
    	
	```


=== "query_log_dhcp_1.json"

    ```json
	
    {
        "message": "Option 82: received a REQUEST DHCP packet from relay-agent eth2 with a circuit-id of \"01:02:03:04:05:06:07:08:09:0a:0b:0c:0d:0e:0f:10\", a remote-id of \"0a:0b:0c:0d\" for 198.51.100.222 (00:11:22:33:44:66) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "REQUEST DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "circuit_id": "01:02:03:04:05:06:07:08:09:0a:0b:0c:0d:0e:0f:10",
                "remote_id": "0a:0b:0c:0d"
            }
        },
        "observer": {
            "egress": {
                "interface": {
                    "name": "eth2"
                }
            }
        },
        "related": {
            "ip": [
                "198.51.100.222"
            ]
        },
        "source": {
            "address": "198.51.100.222",
            "ip": "198.51.100.222",
            "mac": "00:11:22:33:44:66"
        }
    }
    	
	```


=== "query_log_dhcp_10.json"

    ```json
	
    {
        "message": "DHCPOFFER on 203.0.113.4 to 11:22:33:44:55:66 via eth2 relay 203.0.113.8 lease-duration 119 offered-duration 3600",
        "event": {
            "action": "DHCPOFFER",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "lease_time": "119",
                "offered_duration": "3600"
            }
        },
        "observer": {
            "ingress": {
                "interface": {
                    "name": "eth2"
                }
            },
            "ip": "203.0.113.8"
        },
        "related": {
            "ip": [
                "203.0.113.4",
                "203.0.113.8"
            ]
        },
        "source": {
            "address": "203.0.113.4",
            "ip": "203.0.113.4",
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "query_log_dhcp_11.json"

    ```json
	
    {
        "message": "DHCPDISCOVER from 11:22:33:44:55:66 (ABCDEF123) via 203.0.113.4 TransID abcd1234",
        "event": {
            "action": "DHCPDISCOVER",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "ABCDEF123"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "203.0.113.4",
                "trans_id": "abcd1234"
            }
        },
        "observer": {
            "ip": "203.0.113.4"
        },
        "related": {
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "query_log_dhcp_12.json"

    ```json
	
    {
        "message": "DHCPDISCOVER from 11:22:33:44:55:66 (TEST_HOSTNAME) via 203.0.113.4 TransID abcd1234 uid 01:11:22:33:44:55:66",
        "event": {
            "action": "DHCPDISCOVER",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "TEST_HOSTNAME"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "203.0.113.4",
                "trans_id": "abcd1234",
                "uid": "01:11:22:33:44:55:66"
            }
        },
        "observer": {
            "ip": "203.0.113.4"
        },
        "related": {
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "query_log_dhcp_13.json"

    ```json
	
    {
        "message": "DHCPDISCOVER from 11:22:33:44:55:66 via 203.0.113.4 TransID abcd1234",
        "event": {
            "action": "DHCPDISCOVER",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "203.0.113.4",
                "trans_id": "abcd1234"
            }
        },
        "observer": {
            "ip": "203.0.113.4"
        },
        "related": {
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "query_log_dhcp_2.json"

    ```json
	
    {
        "message": "Option 82: received a REQUEST DHCP packet from relay-agent 198.51.100.53 with a circuit-id of \"01:02:03:04:05:06:07:08:09:0a:0b:0c:0d:0e:0f:10\", a remote-id of \"0a:0b:0c:0d\" for 198.51.100.53 (00:11:22:33:44:66) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "REQUEST DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "circuit_id": "01:02:03:04:05:06:07:08:09:0a:0b:0c:0d:0e:0f:10",
                "interface_ip": "198.51.100.53",
                "remote_id": "0a:0b:0c:0d"
            }
        },
        "observer": {
            "ip": "198.51.100.53"
        },
        "related": {
            "ip": [
                "198.51.100.53"
            ]
        },
        "source": {
            "address": "198.51.100.53",
            "ip": "198.51.100.53",
            "mac": "00:11:22:33:44:66"
        }
    }
    	
	```


=== "query_log_dhcp_3.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 198.51.100.107 from 00:11:22:33:44:77 via 198.51.100.107 TransID 80b994d6",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "198.51.100.107",
                "trans_id": "80b994d6"
            }
        },
        "observer": {
            "ip": "198.51.100.107"
        },
        "related": {
            "ip": [
                "198.51.100.107"
            ]
        },
        "source": {
            "address": "198.51.100.107",
            "ip": "198.51.100.107",
            "mac": "00:11:22:33:44:77"
        }
    }
    	
	```


=== "query_log_dhcp_4.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 198.51.100.208 from 00:11:22:33:44:bb (HOST001) via eth2 TransID 823c1fa3 uid 01:00:11:22:33:44:bb (RENEW)",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "outcome": "success",
            "reason": "RENEW",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "HOST001"
        },
        "infoblox": {
            "dhcp": {
                "trans_id": "823c1fa3",
                "uid": "01:00:11:22:33:44:bb"
            }
        },
        "observer": {
            "ingress": {
                "interface": {
                    "name": "eth2"
                }
            }
        },
        "related": {
            "ip": [
                "198.51.100.208"
            ]
        },
        "source": {
            "address": "198.51.100.208",
            "ip": "198.51.100.208",
            "mac": "00:11:22:33:44:bb"
        }
    }
    	
	```


=== "query_log_dhcp_5.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 198.51.100.95 (198.51.100.95) from 00:11:22:33:44:99 via 198.51.100.95 TransID ac1b72c4: lease 198.51.100.95 unavailable.",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "reason": "lease 198.51.100.95 unavailable.",
            "type": [
                "protocol",
                "start"
            ]
        },
        "destination": {
            "address": "198.51.100.95",
            "ip": "198.51.100.95"
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "198.51.100.95",
                "router_ip": "198.51.100.95",
                "trans_id": "ac1b72c4"
            }
        },
        "observer": {
            "ip": "198.51.100.95"
        },
        "related": {
            "ip": [
                "198.51.100.95"
            ]
        },
        "source": {
            "address": "198.51.100.95",
            "ip": "198.51.100.95",
            "mac": "00:11:22:33:44:99"
        }
    }
    	
	```


=== "query_log_dhcp_6.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 198.51.100.159 from 00:11:22:33:44:aa via 198.51.100.159 TransID e711c0c1: ignored (unknown subnet).",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "reason": "ignored (unknown subnet).",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "198.51.100.159",
                "trans_id": "e711c0c1"
            }
        },
        "observer": {
            "ip": "198.51.100.159"
        },
        "related": {
            "ip": [
                "198.51.100.159"
            ]
        },
        "source": {
            "address": "198.51.100.159",
            "ip": "198.51.100.159",
            "mac": "00:11:22:33:44:aa"
        }
    }
    	
	```


=== "query_log_dhcp_7.json"

    ```json
	
    {
        "message": "DHCPACK on 198.51.100.138 to 00:11:22:33:44:88 (HOST002) via eth2 relay 198.51.100.138 lease-duration 172800",
        "event": {
            "action": "DHCPACK",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "HOST002"
        },
        "infoblox": {
            "dhcp": {
                "lease_time": "172800"
            }
        },
        "observer": {
            "ingress": {
                "interface": {
                    "name": "eth2"
                }
            },
            "ip": "198.51.100.138"
        },
        "related": {
            "ip": [
                "198.51.100.138"
            ]
        },
        "source": {
            "address": "198.51.100.138",
            "ip": "198.51.100.138",
            "mac": "00:11:22:33:44:88"
        }
    }
    	
	```


=== "query_log_dhcp_8.json"

    ```json
	
    {
        "message": "r-l-e:198.51.100.113,Fixed,LEASE001,00:11:22:33:44:55,1732119022,1732291822,,$",
        "event": {
            "category": [
                "network"
            ],
            "reason": ",$",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "LEASE001"
        },
        "infoblox": {
            "ddi": {
                "category": "Fixed"
            },
            "dns": {
                "lease_end": "2024-11-22T16:10:22.000000Z",
                "lease_start": "2024-11-20T16:10:22.000000Z"
            }
        },
        "related": {
            "ip": [
                "198.51.100.113"
            ]
        },
        "source": {
            "address": "198.51.100.113",
            "ip": "198.51.100.113",
            "mac": "00:11:22:33:44:55"
        }
    }
    	
	```


=== "query_log_dhcp_9.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 203.0.113.4 from 11:22:33:44:55:66 (ABCDEFGHI) via eth2 TransID 1234abcd (RENEW)",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "outcome": "success",
            "reason": "RENEW",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "ABCDEFGHI"
        },
        "infoblox": {
            "dhcp": {
                "trans_id": "1234abcd"
            }
        },
        "observer": {
            "ingress": {
                "interface": {
                    "name": "eth2"
                }
            }
        },
        "related": {
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "address": "203.0.113.4",
            "ip": "203.0.113.4",
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "query_log_dns_1.json"

    ```json
	
    {
        "message": "FORMERR resolving 'host.example.test/AAAA/IN': 198.51.100.136#53",
        "event": {
            "action": "FORMERR",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "198.51.100.136",
            "ip": "198.51.100.136",
            "port": 53
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "host.example.test",
                "subdomain": "host.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "host.example.test"
            ],
            "ip": [
                "198.51.100.136"
            ]
        }
    }
    	
	```


=== "query_log_dns_2.json"

    ```json
	
    {
        "message": "client 198.51.100.1#1130: UDP: query: example.test IN A response: NXDOMAIN +",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "example.test",
                "subdomain": "example",
                "type": "A"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 1130
        }
    }
    	
	```


=== "query_log_dns_3.json"

    ```json
	
    {
        "message": "client 198.51.100.1#12337: UDP: query: example.test IN A response: NXDOMAIN +AE",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "AA",
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "example.test",
                "subdomain": "example",
                "type": "A"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 12337
        }
    }
    	
	```


=== "query_log_dns_4.json"

    ```json
	
    {
        "message": "client 198.51.100.1#37188: UDP: query: _ldap._tcp.ad.example.test IN SRV response: NXDOMAIN +A",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "AA",
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "_ldap._tcp.ad.example.test",
                "subdomain": "_ldap._tcp.ad.example",
                "type": "SRV"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "_ldap._tcp.ad.example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 37188
        }
    }
    	
	```


=== "query_log_dns_5.json"

    ```json
	
    {
        "message": "client 198.51.100.1#37521: UDP: query: sub.example.test IN AAAA response: NOERROR +A sub.example.test. 86400 IN CNAME sub.example.test.",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "sub.example.test.",
                    "name": "sub.example.test.",
                    "ttl": 86400,
                    "type": "CNAME"
                }
            ],
            "header_flags": [
                "AA",
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "sub.example.test",
                "subdomain": "sub.example",
                "type": "AAAA"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "sub.example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 37521
        }
    }
    	
	```


=== "query_log_dns_6.json"

    ```json
	
    {
        "message": "client 198.51.100.1#40432: UDP: query: app.example.test IN A response: NOERROR + app.example.test. 365 IN A 198.51.100.1; app.example.test. 365 IN A 198.51.100.1; app.example.test. 365 IN A 198.51.100.1; app.example.test. 365 IN A 198.51.100.1; app.example.test. 365 IN A 198.51.100.1; app.example.test. 365 IN A 198.51.100.1",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "app.example.test.",
                    "ttl": 365,
                    "type": "A"
                }
            ],
            "header_flags": [
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "app.example.test",
                "subdomain": "app.example",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "app.example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 40432
        }
    }
    	
	```


=== "query_log_dns_7.json"

    ```json
	
    {
        "message": "client 198.51.100.1#49943: UDP: query: host1.example.test IN A response: NOERROR + host1.example.test. 11720 IN CNAME host1.example.test.; edge1.sub1.cdn.example.test. 67 IN CNAME host1.example.test.; host1.example.test. 52 IN CNAME host1.example.test.; host1.example.test. 235 IN A 198.51.100.1; host1.example.test. 235 IN A 198.51.100.1; host1.example.test. 235 IN A 198.51.100.1; host1.example.test. 235 IN A 198.51.100.1; edge2.cdn.example.test. 235 IN A 198.51.100.1; host1.example.test. 235 IN A 198.51.100.1; host1.example.test. 235 IN A 198.51.100.1; edge2.cdn.example.test. 235 IN A 198.51.100.1;",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "host1.example.test.",
                    "name": "host1.example.test.",
                    "ttl": 11720,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "host1.example.test.",
                    "name": "edge1.sub1.cdn.example.test.",
                    "ttl": 67,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "host1.example.test.",
                    "name": "host1.example.test.",
                    "ttl": 52,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "edge2.cdn.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "host1.example.test.",
                    "ttl": 235,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "198.51.100.1",
                    "name": "edge2.cdn.example.test.",
                    "ttl": 235,
                    "type": "A"
                }
            ],
            "header_flags": [
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "host1.example.test",
                "subdomain": "host1.example",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "host1.example.test"
            ],
            "ip": [
                "198.51.100.1"
            ]
        },
        "source": {
            "address": "198.51.100.1",
            "ip": "198.51.100.1",
            "port": 49943
        }
    }
    	
	```


=== "query_log_dns_8.json"

    ```json
	
    {
        "message": "28-Nov-2024 15:26:27.498 client 203.0.113.4#36615: UDP: query: host1.corp.local IN A response: NOERROR +A host1.example.test. 3600 IN A 198.51.100.201;",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "@timestamp": "2024-11-28T15:26:27.498000Z",
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "198.51.100.201",
                    "name": "host1.example.test.",
                    "ttl": 3600,
                    "type": "A"
                }
            ],
            "header_flags": [
                "AA",
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "host1.corp.local",
                "subdomain": "host1.corp",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "host1.corp.local"
            ],
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "address": "203.0.113.4",
            "ip": "203.0.113.4",
            "port": 36615
        }
    }
    	
	```


=== "query_log_dns_9.json"

    ```json
	
    {
        "message": "28-Nov-2024 15:26:27.359 client 203.0.113.4#63175: UDP: query: www.app.example.test IN A response: NOERROR + www.app.example.test. 7072 IN CNAME www-www.app.cdn.example.test.; www-www.app.cdn.example.test. 56 IN CNAME www.app.edge.example.test.; www.app.edge.example.test. 7154 IN CNAME cache1.cdn.example.test.; cache1.cdn.example.test. 17 IN A 203.0.113.181; cache1.cdn.example.test. 17 IN A 203.0.113.173; cache1.cdn.example.test. 17 IN A 203.0.113.184; cache1.cdn.example.test. 17 IN A 203.0.113.185; cache1.cdn.example.test. 17 IN A 203.0.113.174; cache1.cdn.example.test. 17 IN A 203.0.113.183; cache1.cdn.example.test. 17 IN A 203.0.113.177; cache1.cdn.example.test. 17 IN A 203.0.113.179; cache1.cdn.example.test. 17 IN A 203.0.113.175;",
        "event": {
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "@timestamp": "2024-11-28T15:26:27.359000Z",
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "www-www.app.cdn.example.test.",
                    "name": "www.app.example.test.",
                    "ttl": 7072,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "www.app.edge.example.test.",
                    "name": "www-www.app.cdn.example.test.",
                    "ttl": 56,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "cache1.cdn.example.test.",
                    "name": "www.app.edge.example.test.",
                    "ttl": 7154,
                    "type": "CNAME"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.181",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.173",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.184",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.185",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.174",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.183",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.177",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.179",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                },
                {
                    "class": "IN",
                    "data": "203.0.113.175",
                    "name": "cache1.cdn.example.test.",
                    "ttl": 17,
                    "type": "A"
                }
            ],
            "header_flags": [
                "RA"
            ],
            "question": {
                "class": "IN",
                "name": "www.app.example.test",
                "subdomain": "www.app.example",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "transport": "udp"
        },
        "related": {
            "hosts": [
                "www.app.example.test"
            ],
            "ip": [
                "203.0.113.4"
            ]
        },
        "source": {
            "address": "203.0.113.4",
            "ip": "203.0.113.4",
            "port": 63175
        }
    }
    	
	```


=== "query_log_dnssec.json"

    ```json
	
    {
        "message": "client 192.0.2.66#42811 (ipv6.example.test): query: ipv6.example.test IN A +EDC (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "CD",
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "ipv6.example.test",
                "subdomain": "ipv6.example",
                "type": "A"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "ipv6.example.test"
            ],
            "ip": [
                "192.0.2.66"
            ]
        },
        "source": {
            "address": "192.0.2.66",
            "ip": "192.0.2.66",
            "port": 42811
        }
    }
    	
	```


=== "query_log_edns_version.json"

    ```json
	
    {
        "message": "client @0x7f0000000001 192.0.2.61#38251 (global.vortex.data.example.test): query: global.vortex.data.example.test IN AAAA +E(0) (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "global.vortex.data.example.test",
                "subdomain": "global.vortex.data.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "global.vortex.data.example.test"
            ],
            "ip": [
                "192.0.2.61"
            ]
        },
        "source": {
            "address": "192.0.2.61",
            "ip": "192.0.2.61",
            "port": 38251
        }
    }
    	
	```


=== "query_log_no_recursion.json"

    ```json
	
    {
        "message": "client 192.0.2.66#57980 (ipv6.example.test): query: ipv6.example.test IN AAAA - (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "ipv6.example.test",
                "subdomain": "ipv6.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "ipv6.example.test"
            ],
            "ip": [
                "192.0.2.66"
            ]
        },
        "source": {
            "address": "192.0.2.66",
            "ip": "192.0.2.66",
            "port": 57980
        }
    }
    	
	```


=== "query_log_reverse.json"

    ```json
	
    {
        "message": "client 192.0.2.66#45041 (2.0.0.192.in-addr.arpa): query: 2.0.0.192.in-addr.arpa IN PTR +E (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "2.0.0.192.in-addr.arpa",
                "registered_domain": "192.in-addr.arpa",
                "subdomain": "2.0.0",
                "top_level_domain": "in-addr.arpa",
                "type": "PTR"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "2.0.0.192.in-addr.arpa"
            ],
            "ip": [
                "192.0.2.66"
            ]
        },
        "source": {
            "address": "192.0.2.66",
            "ip": "192.0.2.66",
            "port": 45041
        }
    }
    	
	```


=== "query_log_tcp.json"

    ```json
	
    {
        "message": "client 192.0.2.70#55575 (app.example.test): query: app.example.test IN AAAA +ET (192.0.2.102)",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [
                "RD"
            ],
            "question": {
                "class": "IN",
                "name": "app.example.test",
                "subdomain": "app.example",
                "type": "AAAA"
            },
            "type": "query"
        },
        "network": {
            "transport": "tcp"
        },
        "related": {
            "hosts": [
                "app.example.test"
            ],
            "ip": [
                "192.0.2.70"
            ]
        },
        "source": {
            "address": "192.0.2.70",
            "ip": "192.0.2.70",
            "port": 55575
        }
    }
    	
	```


=== "test_admin_login_allowed.json"

    ```json
	
    {
        "message": "2026-03-02 16:41:03.302Z [adminsec]: Login_Allowed - - to=AdminConnector ip=192.0.2.50 auth=LOCAL group=G-ADMIN apparently_via=API",
        "event": {
            "action": "Login_Allowed",
            "category": [
                "authentication"
            ],
            "outcome": "success",
            "type": [
                "start"
            ]
        },
        "@timestamp": "2026-03-02T16:41:03.302000Z",
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "ip": [
                "192.0.2.50"
            ]
        },
        "source": {
            "address": "192.0.2.50",
            "ip": "192.0.2.50"
        }
    }
    	
	```


=== "test_admin_login_allowed_2.json"

    ```json
	
    {
        "message": "2026-07-16 09:15:53.317Z [adminsec]: Login_Allowed - - to=AdminConnector ip=192.0.2.4 auth=LOCAL group=GROUP-TEST apparently_via=API",
        "event": {
            "action": "Login_Allowed",
            "category": [
                "authentication"
            ],
            "outcome": "success",
            "type": [
                "start"
            ]
        },
        "@timestamp": "2026-07-16T09:15:53.317000Z",
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "ip": [
                "192.0.2.4"
            ]
        },
        "source": {
            "address": "192.0.2.4",
            "ip": "192.0.2.4"
        }
    }
    	
	```


=== "test_cef_dns_query_a.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Query|DNS Query IN A|1|dst=192.0.2.1 src=198.51.100.10 spt=46842 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=host1.in.example.net InfobloxDNSQClass=IN InfobloxDNSQType=A InfobloxDNSQFlags=+ InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Query",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "host1.in.example.net",
                "registered_domain": "example.net",
                "subdomain": "host1.in",
                "top_level_domain": "net",
                "type": "A"
            },
            "type": "query"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "host1.in.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.10"
            ]
        },
        "source": {
            "address": "198.51.100.10",
            "ip": "198.51.100.10",
            "port": 46842
        }
    }
    	
	```


=== "test_cef_dns_query_aaaa.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Query|DNS Query IN AAAA|1|dst=192.0.2.1 src=198.51.100.20 spt=36442 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=proxy-app.example.net InfobloxDNSQClass=IN InfobloxDNSQType=AAAA InfobloxDNSQFlags=+ InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Query",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "proxy-app.example.net",
                "registered_domain": "example.net",
                "subdomain": "proxy-app",
                "top_level_domain": "net",
                "type": "AAAA"
            },
            "type": "query"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "proxy-app.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.20"
            ]
        },
        "source": {
            "address": "198.51.100.20",
            "ip": "198.51.100.20",
            "port": 36442
        }
    }
    	
	```


=== "test_cef_dns_query_ptr.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Query|DNS Query IN PTR|1|dst=192.0.2.1 src=198.51.100.40 spt=37690 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=2.1.254.169.in-addr.arpa InfobloxDNSQClass=IN InfobloxDNSQType=PTR InfobloxDNSQFlags=+ InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Query",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "2.1.254.169.in-addr.arpa",
                "registered_domain": "169.in-addr.arpa",
                "subdomain": "2.1.254",
                "top_level_domain": "in-addr.arpa",
                "type": "PTR"
            },
            "type": "query"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "2.1.254.169.in-addr.arpa"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.40"
            ]
        },
        "source": {
            "address": "198.51.100.40",
            "ip": "198.51.100.40",
            "port": 37690
        }
    }
    	
	```


=== "test_cef_dns_query_srv.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Query|DNS Query IN SRV|1|dst=192.0.2.1 src=198.51.100.30 spt=44481 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=_gc._tcp.ad.example.net InfobloxDNSQClass=IN InfobloxDNSQType=SRV InfobloxDNSQFlags=+ InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Query",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "_gc._tcp.ad.example.net",
                "registered_domain": "example.net",
                "subdomain": "_gc._tcp.ad",
                "top_level_domain": "net",
                "type": "SRV"
            },
            "type": "query"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "_gc._tcp.ad.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.30"
            ]
        },
        "source": {
            "address": "198.51.100.30",
            "ip": "198.51.100.30",
            "port": 44481
        }
    }
    	
	```


=== "test_cef_dns_query_txt.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Query|DNS Query IN TXT|1|dst=192.0.2.1 src=198.51.100.50 spt=44831 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=_nfsv4idmapdomain.in.example.net InfobloxDNSQClass=IN InfobloxDNSQType=TXT InfobloxDNSQFlags=+ InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Query",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "_nfsv4idmapdomain.in.example.net",
                "registered_domain": "example.net",
                "subdomain": "_nfsv4idmapdomain.in",
                "top_level_domain": "net",
                "type": "TXT"
            },
            "type": "query"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "_nfsv4idmapdomain.in.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.50"
            ]
        },
        "source": {
            "address": "198.51.100.50",
            "ip": "198.51.100.50",
            "port": 44831
        }
    }
    	
	```


=== "test_cef_dns_response_a_noerror.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN A NOERROR|1|dst=192.0.2.1 src=198.51.100.10 spt=46842 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=host1.in.example.net InfobloxDNSQClass=IN InfobloxDNSQType=A InfobloxDNSQFlags=+A InfobloxDNSRCode=NOERROR msg=\"host1.in.example.net. 3600 IN A 198.51.100.11\" InfobloxAnCount=1 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "198.51.100.11",
                    "name": "host1.in.example.net.",
                    "ttl": 3600,
                    "type": "A"
                }
            ],
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "host1.in.example.net",
                "registered_domain": "example.net",
                "subdomain": "host1.in",
                "top_level_domain": "net",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "host1.in.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.10"
            ]
        },
        "source": {
            "address": "198.51.100.10",
            "ip": "198.51.100.10",
            "port": 46842
        }
    }
    	
	```


=== "test_cef_dns_response_a_noerror_no_msg.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN A NOERROR|1|dst=192.0.2.1 src=127.0.0.1 spt=36615 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=. InfobloxDNSQClass=IN InfobloxDNSQType=A InfobloxDNSQFlags=+ InfobloxDNSRCode=NOERROR InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": ".",
                "type": "A"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "."
            ],
            "ip": [
                "127.0.0.1",
                "192.0.2.1"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1",
            "port": 36615
        }
    }
    	
	```


=== "test_cef_dns_response_a_nxdomain.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN A NXDOMAIN|1|dst=192.0.2.1 src=198.51.100.60 spt=42552 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=host2.in.example.net.ex.example.net InfobloxDNSQClass=IN InfobloxDNSQType=A InfobloxDNSQFlags=+A InfobloxDNSRCode=NXDOMAIN InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "host2.in.example.net.ex.example.net",
                "registered_domain": "example.net",
                "subdomain": "host2.in.example.net.ex",
                "top_level_domain": "net",
                "type": "A"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "host2.in.example.net.ex.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.60"
            ]
        },
        "source": {
            "address": "198.51.100.60",
            "ip": "198.51.100.60",
            "port": 42552
        }
    }
    	
	```


=== "test_cef_dns_response_aaaa_noerror.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN AAAA NOERROR|1|dst=192.0.2.1 src=198.51.100.20 spt=36442 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=proxy-app.example.net InfobloxDNSQClass=IN InfobloxDNSQType=AAAA InfobloxDNSQFlags=+A InfobloxDNSRCode=NOERROR InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "proxy-app.example.net",
                "registered_domain": "example.net",
                "subdomain": "proxy-app",
                "top_level_domain": "net",
                "type": "AAAA"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "proxy-app.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.20"
            ]
        },
        "source": {
            "address": "198.51.100.20",
            "ip": "198.51.100.20",
            "port": 36442
        }
    }
    	
	```


=== "test_cef_dns_response_aaaa_nxdomain.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN AAAA NXDOMAIN|1|dst=192.0.2.1 src=198.51.100.60 spt=5183 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=host2.in.example.net.cloud.example.com InfobloxDNSQClass=IN InfobloxDNSQType=AAAA InfobloxDNSQFlags=+ InfobloxDNSRCode=NXDOMAIN InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "host2.in.example.net.cloud.example.com",
                "registered_domain": "example.com",
                "subdomain": "host2.in.example.net.cloud",
                "top_level_domain": "com",
                "type": "AAAA"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "host2.in.example.net.cloud.example.com"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.60"
            ]
        },
        "source": {
            "address": "198.51.100.60",
            "ip": "198.51.100.60",
            "port": 5183
        }
    }
    	
	```


=== "test_cef_dns_response_ptr_noerror.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN PTR NOERROR|1|dst=192.0.2.2 src=127.0.0.1 spt=46588 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=1.0.0.127.in-addr.arpa InfobloxDNSQClass=IN InfobloxDNSQType=PTR InfobloxDNSQFlags=+A InfobloxDNSRCode=NOERROR msg=\"1.0.0.127.in-addr.arpa. 3600 IN PTR localhost.\" InfobloxAnCount=1 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "success",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.2",
            "ip": "192.0.2.2"
        },
        "dns": {
            "answers": [
                {
                    "class": "IN",
                    "data": "localhost.",
                    "name": "1.0.0.127.in-addr.arpa.",
                    "ttl": 3600,
                    "type": "PTR"
                }
            ],
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "1.0.0.127.in-addr.arpa",
                "registered_domain": "127.in-addr.arpa",
                "subdomain": "1.0.0",
                "top_level_domain": "in-addr.arpa",
                "type": "PTR"
            },
            "response_code": "NOERROR",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "1.0.0.127.in-addr.arpa"
            ],
            "ip": [
                "127.0.0.1",
                "192.0.2.2"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1",
            "port": 46588
        }
    }
    	
	```


=== "test_cef_dns_response_ptr_nxdomain.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN PTR NXDOMAIN|1|dst=192.0.2.1 src=198.51.100.40 spt=37690 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=2.1.254.169.in-addr.arpa InfobloxDNSQClass=IN InfobloxDNSQType=PTR InfobloxDNSQFlags=+ InfobloxDNSRCode=NXDOMAIN InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "2.1.254.169.in-addr.arpa",
                "registered_domain": "169.in-addr.arpa",
                "subdomain": "2.1.254",
                "top_level_domain": "in-addr.arpa",
                "type": "PTR"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "2.1.254.169.in-addr.arpa"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.40"
            ]
        },
        "source": {
            "address": "198.51.100.40",
            "ip": "198.51.100.40",
            "port": 37690
        }
    }
    	
	```


=== "test_cef_dns_response_srv_nxdomain.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN SRV NXDOMAIN|1|dst=192.0.2.1 src=198.51.100.30 spt=55893 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=_gc._tcp.ad.example.net InfobloxDNSQClass=IN InfobloxDNSQType=SRV InfobloxDNSQFlags=+ InfobloxDNSRCode=NXDOMAIN InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "_gc._tcp.ad.example.net",
                "registered_domain": "example.net",
                "subdomain": "_gc._tcp.ad",
                "top_level_domain": "net",
                "type": "SRV"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "_gc._tcp.ad.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.30"
            ]
        },
        "source": {
            "address": "198.51.100.30",
            "ip": "198.51.100.30",
            "port": 55893
        }
    }
    	
	```


=== "test_cef_dns_response_txt_nxdomain.json"

    ```json
	
    {
        "message": "CEF:0|Infoblox|Data Connector|2.1.3|DNS Response|DNS Response IN TXT NXDOMAIN|1|dst=192.0.2.1 src=198.51.100.50 spt=44831 proto=UDP app=DNS InfobloxDNSView=Internal destinationDnsDomain=_nfsv4idmapdomain.in.example.net InfobloxDNSQClass=IN InfobloxDNSQType=TXT InfobloxDNSQFlags=+A InfobloxDNSRCode=NXDOMAIN InfobloxAnCount=0 InfobloxNsCount=0 InfobloxArCount=0",
        "event": {
            "action": "DNS Response",
            "category": [
                "network"
            ],
            "outcome": "failure",
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "_nfsv4idmapdomain.in.example.net",
                "registered_domain": "example.net",
                "subdomain": "_nfsv4idmapdomain.in",
                "top_level_domain": "net",
                "type": "TXT"
            },
            "response_code": "NXDOMAIN",
            "type": "answer"
        },
        "network": {
            "protocol": "dns",
            "transport": "udp"
        },
        "observer": {
            "product": "Data Connector",
            "vendor": "Infoblox",
            "version": "2.1.3"
        },
        "related": {
            "hosts": [
                "_nfsv4idmapdomain.in.example.net"
            ],
            "ip": [
                "192.0.2.1",
                "198.51.100.50"
            ]
        },
        "source": {
            "address": "198.51.100.50",
            "ip": "198.51.100.50",
            "port": 44831
        }
    }
    	
	```


=== "test_dhcp_balanced_pool.json"

    ```json
	
    {
        "message": "balanced pool 563a3ae5f670 192.0.2.64/28  total 11  free 0  backup 1  lts 0  max-misbal 0  queue 1",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "pool": {
                    "id": "563a3ae5f670",
                    "subnet": "192.0.2.64/28"
                }
            }
        }
    }
    	
	```


=== "test_dhcp_balancing_pool.json"

    ```json
	
    {
        "message": "balancing pool 563a3ae5f670 192.0.2.64/28  total 11  free 1  backup 0  lts 1  max-own (+/-)0",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "pool": {
                    "id": "563a3ae5f670",
                    "subnet": "192.0.2.64/28"
                }
            }
        }
    }
    	
	```


=== "test_dhcp_bootrequest.json"

    ```json
	
    {
        "message": "BOOTREQUEST from 11:22:33:44:55:66 via 192.0.2.4: BOOTP from dynamic client and no dynamic leases",
        "event": {
            "action": "BOOTREQUEST",
            "category": [
                "network"
            ],
            "reason": "BOOTP from dynamic client and no dynamic leases",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "192.0.2.4"
            }
        },
        "observer": {
            "ip": "192.0.2.4"
        },
        "related": {
            "ip": [
                "192.0.2.4"
            ]
        },
        "source": {
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "test_dhcp_expire.json"

    ```json
	
    {
        "message": "DHCPEXPIRE on 192.0.2.100 to 00:11:22:33:44:cc",
        "event": {
            "action": "DHCPEXPIRE",
            "category": [
                "network"
            ],
            "type": [
                "end",
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "ip": [
                "192.0.2.100"
            ]
        },
        "source": {
            "address": "192.0.2.100",
            "ip": "192.0.2.100",
            "mac": "00:11:22:33:44:cc"
        }
    }
    	
	```


=== "test_dhcp_failover_peer.json"

    ```json
	
    {
        "message": "failover peer DHCP-FAILOVER(1770103776p): 1 leases added to send queue from pool 563a3ae5f670 192.0.2.64/28",
        "event": {
            "category": [
                "network"
            ],
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "failover_peer": "DHCP-FAILOVER(1770103776p)",
                "pool": {
                    "id": "563a3ae5f670",
                    "subnet": "192.0.2.64/28"
                }
            }
        }
    }
    	
	```


=== "test_dhcp_inform_not_authoritative.json"

    ```json
	
    {
        "message": "DHCPINFORM from 192.0.2.50 via 192.0.2.2 TransID 34df1cdc: not authoritative for subnet 192.0.2.0",
        "event": {
            "action": "DHCPINFORM",
            "category": [
                "network"
            ],
            "reason": "not authoritative for subnet 192.0.2.0",
            "type": [
                "protocol"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "192.0.2.2",
                "trans_id": "34df1cdc"
            }
        },
        "observer": {
            "ip": "192.0.2.2"
        },
        "related": {
            "ip": [
                "192.0.2.2",
                "192.0.2.50"
            ]
        },
        "source": {
            "address": "192.0.2.50",
            "ip": "192.0.2.50"
        }
    }
    	
	```


=== "test_dhcp_option82_discover.json"

    ```json
	
    {
        "message": "Option 82: received a DISCOVER DHCP packet from relay-agent 203.0.113.1 with a link-selection of \"203.0.113.0\", a server-id-override of \"203.0.113.2\" for 203.0.113.10 (00:11:22:33:44:dd) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "DISCOVER DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "203.0.113.1",
                "link_selection": "203.0.113.0",
                "server_id_override": "203.0.113.2"
            }
        },
        "observer": {
            "ip": "203.0.113.1"
        },
        "related": {
            "ip": [
                "203.0.113.1",
                "203.0.113.10"
            ]
        },
        "source": {
            "address": "203.0.113.10",
            "ip": "203.0.113.10",
            "mac": "00:11:22:33:44:dd"
        }
    }
    	
	```


=== "test_dhcp_option82_discover_circuit_id.json"

    ```json
	
    {
        "message": "Option 82: received a DISCOVER DHCP packet from relay-agent 203.0.113.3 with a circuit-id of \"00:04:01:02:03:04\", a link-selection of \"203.0.113.0\", a server-id-override of \"203.0.113.2\" for 203.0.113.11 (00:11:22:33:44:dd) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "DISCOVER DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "circuit_id": "00:04:01:02:03:04",
                "interface_ip": "203.0.113.3",
                "link_selection": "203.0.113.0",
                "server_id_override": "203.0.113.2"
            }
        },
        "observer": {
            "ip": "203.0.113.3"
        },
        "related": {
            "ip": [
                "203.0.113.11",
                "203.0.113.3"
            ]
        },
        "source": {
            "address": "203.0.113.11",
            "ip": "203.0.113.11",
            "mac": "00:11:22:33:44:dd"
        }
    }
    	
	```


=== "test_dhcp_option82_request_circuit_link.json"

    ```json
	
    {
        "message": "Option 82: received a REQUEST DHCP packet from relay-agent 203.0.113.5 with a circuit-id of \"00:04:05:06:07:08\", a link-selection of \"203.0.113.128\", a server-id-override of \"203.0.113.129\" for 203.0.113.131 (00:11:22:33:44:ee) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "REQUEST DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "circuit_id": "00:04:05:06:07:08",
                "interface_ip": "203.0.113.5",
                "link_selection": "203.0.113.128",
                "server_id_override": "203.0.113.129"
            }
        },
        "observer": {
            "ip": "203.0.113.5"
        },
        "related": {
            "ip": [
                "203.0.113.131",
                "203.0.113.5"
            ]
        },
        "source": {
            "address": "203.0.113.131",
            "ip": "203.0.113.131",
            "mac": "00:11:22:33:44:ee"
        }
    }
    	
	```


=== "test_dhcp_option82_request_link_selection.json"

    ```json
	
    {
        "message": "Option 82: received a REQUEST DHCP packet from relay-agent 203.0.113.4 with a link-selection of \"203.0.113.128\", a server-id-override of \"203.0.113.129\" for 203.0.113.130 (00:11:22:33:44:ee) lease time is undefined seconds. (NEW)",
        "event": {
            "action": "REQUEST DHCP",
            "category": [
                "network"
            ],
            "reason": "lease time is undefined seconds. (NEW)",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "203.0.113.4",
                "link_selection": "203.0.113.128",
                "server_id_override": "203.0.113.129"
            }
        },
        "observer": {
            "ip": "203.0.113.4"
        },
        "related": {
            "ip": [
                "203.0.113.130",
                "203.0.113.4"
            ]
        },
        "source": {
            "address": "203.0.113.130",
            "ip": "203.0.113.130",
            "mac": "00:11:22:33:44:ee"
        }
    }
    	
	```


=== "test_dhcpack.json"

    ```json
	
    {
        "message": "DHCPACK on 192.0.2.4 to 11:22:33:44:55:66 (ABCDE1234) via bond0 relay 203.0.113.40 lease-duration 43200 (RENEW) uid 01:11:22:33:44:55:66",
        "event": {
            "action": "DHCPACK",
            "category": [
                "network"
            ],
            "outcome": "success",
            "reason": "RENEW",
            "type": [
                "protocol",
                "start"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "ABCDE1234"
        },
        "infoblox": {
            "dhcp": {
                "lease_time": "43200",
                "uid": "01:11:22:33:44:55:66"
            }
        },
        "observer": {
            "ingress": {
                "interface": {
                    "name": "bond0"
                }
            },
            "ip": "203.0.113.40"
        },
        "related": {
            "ip": [
                "192.0.2.4",
                "203.0.113.40"
            ]
        },
        "source": {
            "address": "192.0.2.4",
            "ip": "192.0.2.4",
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "test_dhcprequest.json"

    ```json
	
    {
        "message": "DHCPREQUEST for 192.0.2.4 (203.0.113.40) from 11:22:33:44:55:66 (ABCD1234) via 198.51.100.8 TransID 11111111 uid 01:11:22:33:44:55:66",
        "event": {
            "action": "DHCPREQUEST",
            "category": [
                "network"
            ],
            "type": [
                "protocol",
                "start"
            ]
        },
        "destination": {
            "address": "203.0.113.40",
            "ip": "203.0.113.40"
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "host": {
            "name": "ABCD1234"
        },
        "infoblox": {
            "dhcp": {
                "interface_ip": "198.51.100.8",
                "router_ip": "203.0.113.40",
                "trans_id": "11111111",
                "uid": "01:11:22:33:44:55:66"
            }
        },
        "observer": {
            "ip": "198.51.100.8"
        },
        "related": {
            "ip": [
                "192.0.2.4",
                "198.51.100.8",
                "203.0.113.40"
            ]
        },
        "source": {
            "address": "192.0.2.4",
            "ip": "192.0.2.4",
            "mac": "11:22:33:44:55:66"
        }
    }
    	
	```


=== "test_dns_bind_update_rejected.json"

    ```json
	
    {
        "message": "bind update on 192.0.2.27 from DHCP-FAILOVER(1770103776p) rejected: incoming update is less critical than outgoing update",
        "event": {
            "category": [
                "network"
            ],
            "reason": "incoming update is less critical than outgoing update",
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "hosts": [
                "DHCP-FAILOVER(1770103776p)"
            ],
            "ip": [
                "192.0.2.27"
            ]
        },
        "source": {
            "address": "DHCP-FAILOVER(1770103776p)",
            "domain": "DHCP-FAILOVER(1770103776p)",
            "ip": "192.0.2.27"
        }
    }
    	
	```


=== "test_dns_bind_update_rejected_2.json"

    ```json
	
    {
        "message": "bind update on 192.0.2.4 from TESTORG-DHCP-FAILOVER(1784109718p) rejected: incoming update is less critical than outgoing update",
        "event": {
            "category": [
                "network"
            ],
            "reason": "incoming update is less critical than outgoing update",
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "hosts": [
                "TESTORG-DHCP-FAILOVER(1784109718p)"
            ],
            "ip": [
                "192.0.2.4"
            ]
        },
        "source": {
            "address": "TESTORG-DHCP-FAILOVER(1784109718p)",
            "domain": "TESTORG-DHCP-FAILOVER(1784109718p)",
            "ip": "192.0.2.4"
        }
    }
    	
	```


=== "test_dns_format_error.json"

    ```json
	
    {
        "message": "DNS format error 192.0.2.4#53 resolving ns1.example.net/NS for 198.51.100.8#40815: Name test.net (SOA) not subdomain of zone example.net --invalid response",
        "event": {
            "category": [
                "network"
            ],
            "reason": "Name test.net (SOA) not subdomain of zone example.net --invalid response",
            "type": [
                "info"
            ]
        },
        "destination": {
            "address": "198.51.100.8",
            "ip": "198.51.100.8",
            "port": 40815
        },
        "dns": {
            "header_flags": [],
            "question": {
                "name": "ns1.example.net",
                "registered_domain": "example.net",
                "subdomain": "ns1",
                "top_level_domain": "net",
                "type": "NS"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "ns1.example.net"
            ],
            "ip": [
                "192.0.2.4",
                "198.51.100.8"
            ]
        },
        "source": {
            "address": "192.0.2.4",
            "ip": "192.0.2.4",
            "port": 53
        }
    }
    	
	```


=== "test_dns_internal_send_error.json"

    ```json
	
    {
        "message": "internal_send: 192.0.2.231#53: Invalid argument",
        "event": {
            "category": [
                "network"
            ],
            "reason": "Invalid argument",
            "type": [
                "info"
            ]
        },
        "destination": {
            "address": "192.0.2.231",
            "ip": "192.0.2.231",
            "port": 53
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "related": {
            "ip": [
                "192.0.2.231"
            ]
        }
    }
    	
	```


=== "test_dns_refused_rcode.json"

    ```json
	
    {
        "message": "REFUSED unexpected RCODE resolving 'maps.example.com/A/IN': 203.0.113.99#53",
        "event": {
            "action": "REFUSED",
            "category": [
                "network"
            ],
            "type": [
                "protocol"
            ]
        },
        "destination": {
            "address": "203.0.113.99",
            "ip": "203.0.113.99",
            "port": 53
        },
        "dns": {
            "header_flags": [],
            "question": {
                "class": "IN",
                "name": "maps.example.com",
                "registered_domain": "example.com",
                "subdomain": "maps",
                "top_level_domain": "com",
                "type": "A"
            },
            "type": "query"
        },
        "related": {
            "hosts": [
                "maps.example.com"
            ],
            "ip": [
                "203.0.113.99"
            ]
        }
    }
    	
	```


=== "test_dns_socket_error.json"

    ```json
	
    {
        "message": "socket.c:1771: unexpected error:",
        "event": {
            "category": [
                "network"
            ],
            "reason": "unexpected error:",
            "type": [
                "info"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        }
    }
    	
	```


=== "test_storage_log_file_deleted.json"

    ```json
	
    {
        "message": "Log file /storage/reporting-capture-data/captured-dns-ns1-1771869582-1771869642.gz was deleted cause of logs directory overflow.",
        "event": {
            "category": [
                "file"
            ],
            "outcome": "success",
            "reason": "logs directory overflow.",
            "type": [
                "deletion"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "file": {
            "directory": "/storage/reporting-capture-data",
            "name": "captured-dns-ns1-1771869582-1771869642.gz",
            "path": "/storage/reporting-capture-data/captured-dns-ns1-1771869582-1771869642.gz"
        }
    }
    	
	```


=== "test_storage_md5_file_deleted.json"

    ```json
	
    {
        "message": "MD5 file /storage/reporting-capture-data/captured-dns-ns1-1771869582-1771869642.gz.md5 was deleted cause of logs directory overflow.",
        "event": {
            "category": [
                "file"
            ],
            "outcome": "success",
            "reason": "logs directory overflow.",
            "type": [
                "deletion"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "file": {
            "directory": "/storage/reporting-capture-data",
            "name": "captured-dns-ns1-1771869582-1771869642.gz.md5",
            "path": "/storage/reporting-capture-data/captured-dns-ns1-1771869582-1771869642.gz.md5"
        }
    }
    	
	```


=== "test_storage_uploaded_file_deleted.json"

    ```json
	
    {
        "message": "Uploaded file /storage/tmp/captured-dns-ns1-1771869582-1771869642.gz was deleted cause of logs directory overflow.",
        "event": {
            "category": [
                "file"
            ],
            "outcome": "success",
            "reason": "logs directory overflow.",
            "type": [
                "deletion"
            ]
        },
        "dns": {
            "header_flags": [],
            "type": "query"
        },
        "file": {
            "directory": "/storage/tmp",
            "name": "captured-dns-ns1-1771869582-1771869642.gz",
            "path": "/storage/tmp/captured-dns-ns1-1771869582-1771869642.gz"
        }
    }
    	
	```





### Extracted Fields

The following table lists the fields that are extracted, normalized under the ECS format, analyzed and indexed by the parser. It should be noted that infered fields are not listed.

| Name | Type | Description                |
| ---- | ---- | ---------------------------|
|`@timestamp` | `date` | Date/time when the event originated. |
|`destination.ip` | `ip` | IP address of the destination. |
|`destination.port` | `long` | Port of the destination. |
|`dns.answers` | `object` | Array of DNS answers. |
|`dns.header_flags` | `keyword` | Array of DNS header flags. |
|`dns.question.class` | `keyword` | The class of records being queried. |
|`dns.question.name` | `keyword` | The name being queried. |
|`dns.question.type` | `keyword` | The type of record being queried. |
|`dns.response_code` | `keyword` | The DNS response code. |
|`dns.type` | `keyword` | The type of DNS event captured, query or answer. |
|`event.action` | `keyword` | The action captured by the event. |
|`event.category` | `keyword` | Event category. The second categorization field in the hierarchy. |
|`event.outcome` | `keyword` | The outcome of the event. The lowest level categorization field in the hierarchy. |
|`event.reason` | `keyword` | Reason why this event happened, according to the source |
|`event.type` | `keyword` | Event type. The third categorization field in the hierarchy. |
|`file.path` | `keyword` | Full path to the file, including the file name. |
|`host.name` | `keyword` | Name of the host. |
|`infoblox.ddi.category` | `keyword` | The logging category of this event. |
|`infoblox.dhcp.circuit_id` | `keyword` | The circuit ID. |
|`infoblox.dhcp.failover_peer` | `keyword` | The failover peer. |
|`infoblox.dhcp.interface_ip` | `ip` | The IP address of the interface. |
|`infoblox.dhcp.lease_time` | `keyword` | The lease time. |
|`infoblox.dhcp.link_selection` | `keyword` | The link selection. |
|`infoblox.dhcp.offered_duration` | `keyword` | The offered duration of the lease. |
|`infoblox.dhcp.pool.id` | `keyword` | The DHCP pool id. |
|`infoblox.dhcp.pool.subnet` | `keyword` | The subnet of the DHCP pool. |
|`infoblox.dhcp.remote_id` | `keyword` | The remote ID. |
|`infoblox.dhcp.router_ip` | `ip` | The IP address of the router. |
|`infoblox.dhcp.server_id_override` | `keyword` | The server ID override. |
|`infoblox.dhcp.trans_id` | `keyword` | The transaction ID. |
|`infoblox.dhcp.uid` | `keyword` | The unique identifier for the DHCP lease. |
|`infoblox.dns.lease_end` | `date` | The end time of the lease. |
|`infoblox.dns.lease_start` | `date` | The start time of the lease. |
|`network.protocol` | `keyword` | Application protocol name. |
|`network.transport` | `keyword` | Protocol Name corresponding to the field `iana_number`. |
|`observer.egress.interface.name` | `keyword` | Interface name |
|`observer.ingress.interface.name` | `keyword` | Interface name |
|`observer.ip` | `ip` | IP addresses of the observer. |
|`observer.product` | `keyword` | The product name of the observer. |
|`observer.vendor` | `keyword` | Vendor name of the observer. |
|`observer.version` | `keyword` | Observer version. |
|`server.ip` | `ip` | IP address of the server. |
|`source.domain` | `keyword` | The domain name of the source. |
|`source.ip` | `ip` | IP address of the source. |
|`source.mac` | `keyword` | MAC address of the source. |
|`source.port` | `long` | Port of the source. |



For more information on the Intake Format, please find the code of the Parser, Smart Descriptions, and Supported Events [here](https://github.com/SEKOIA-IO/intake-formats/tree/main/Infoblox/ddi).