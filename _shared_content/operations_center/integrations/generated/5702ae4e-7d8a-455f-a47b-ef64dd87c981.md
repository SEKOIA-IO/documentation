...
|`fortinet.fortigate.session.duration` | `long` | Duration in seconds between the last session log and the current session log |
|`fortinet.fortigate.session.received_bytes` | `long` | Increment of bytes which are received after the start of the session |
|`fortinet.fortigate.session.received_packets` | `long` | Increment of packets which are received after the start of the session |
|`fortinet.fortigate.session.sent_bytes` | `long` | Increment of bytes which are sent after the start of the session |
|`fortinet.fortigate.session.sent_packets` | `long` | Increment of packets which are sent after the start of the session |
