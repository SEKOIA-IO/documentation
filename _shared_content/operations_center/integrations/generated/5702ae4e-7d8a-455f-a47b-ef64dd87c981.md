...
=== "event_incident_detection.STANDARD.json"

    ```json
	
    {
        "message": "version=1.2 devhost=decoy-sensor01 devid=FGTVM0000000001 logver=25 tzone=0 tz=UTC date=2026-07-06 time=10:10:37 logid=0101000068 type=event subtype=attack level=alert user=system ui=GUI action=Incident_Detection status=success loghost=collector01 msg=\"EventID=9216904394757402465 IncidentID=9216901844840843855 Tagkey=198.51.100.42:61230:203.0.113.86:80:1158648005tag t231899233-9216901844840843855 DecoyGroup=IT_Decoy DecoyType=Linux MITRE_ATT&CK_ID=T1110 AttackerIP=198.51.100.42 AttackerPort=61230 VictimIP=203.0.113.86 VictimPort=80 Operation=Close_Port Severity=medium TagID=0 AttackerMac=02:42:ac:11:00:05 Service=HTTP Username=decoy-user Password=REDACTED Description=\\\"Credential_reset_required\\\"\"",
        "event": {
            "action": "Close_Port",
            "category": "event",
            "code": "0101000068",
            "outcome": "success",
            "reason": "Credential_reset_required",
            "timezone": "UTC"
        },
        "@timestamp": "2026-07-06T10:10:37Z",
        "action": {
            "name": "Close_Port",
            "outcome": "success",
            "outcome_reason": "Credential_reset_required",
            "target": "network-traffic",
            "type": "attack"
        },
        "destination": {
        ...
        "fortinet": {
            "fortigate": {
                ...
                "incident": {
                    "event_id": "9216904394757402465",
                    "id": "9216901844840843855"
                },
                "log_version": "25",
                "loghost": "collector01",
        ...
        "observer": {
            "hostname": "decoy-sensor01",
            "serial_number": "FGTVM0000000001",
            "version": "1.2"
        },
        "related": {
            "hosts": [
...
=== "test_ssl_certificate_error.json"
...
=== "vpn_xauth_identity.STANDARD.json"

    ```json
	
    {
        "message": "eventtime=1741264747 tz=\"+0000\" logid=\"0101037142\" type=\"event\" subtype=\"vpn\" level=\"notice\" vd=\"root\" logdesc=\"IPsec connection status changed\" msg=\"IPsec connection status change\" action=\"tunnel-up\" remip=198.51.100.24 locip=192.0.2.45 remport=4500 locport=4500 outintf=\"port2\" srccountry=\"Exampleland\" cookies=\"abcdef01234/abcdef01234\" user=\"198.51.100.24\" group=\"N/A\" useralt=\"N/A\" xauthuser=\"analyst@example.org\" xauthgroup=\"remote_users\" assignip=203.0.113.77 vpntunnel=\"VPN_TUNNEL_A\" tunnelip=203.0.113.77 tunnelid=123456789 tunneltype=\"ipsec\" duration=0 sentbyte=0 rcvdbyte=0 nextstat=0 fctuid=\"2F256A2D11CC55E2D2A60D8C8A5F\" advpnsc=0",
        "event": {
            "action": "tunnel-up",
            "category": "event",
            "code": "0101037142",
            "dataset": "event:vpn",
            "outcome": "success",
            "reason": "IPsec connection status change",
            "timezone": "+0000"
        },
        "@timestamp": "2025-03-06T12:39:07Z",
        "action": {
            "name": "tunnel-up",
            "outcome": "success",
            "outcome_reason": "IPsec connection status change",
            "target": "network-traffic",
            "type": "vpn"
        },
        "destination": {
...
|`fortinet.fortigate.decoy.group` | `keyword` | Decoy group associated with the incident detection event. |
|`fortinet.fortigate.decoy.type` | `keyword` | Decoy type associated with the incident detection event. |
...
|`fortinet.fortigate.incident.event_id` | `keyword` | Event identifier included in incident detection messages. |
|`fortinet.fortigate.incident.id` | `keyword` | Incident identifier included in incident detection messages. |
...
|`fortinet.fortigate.loghost` | `keyword` | Logging host as reported by Fortigate. |
|`fortinet.fortigate.method` | `keyword` |  |
|`fortinet.fortigate.nat_translation_type` | `keyword` | NAT translation type. |
|`fortinet.fortigate.operation` | `keyword` | Operation performed during incident detection. |
|`fortinet.fortigate.password` | `keyword` | Password value reported by the incident detection payload. |
...
|`fortinet.fortigate.tag.id` | `keyword` | Tag identifier associated with the incident. |
|`fortinet.fortigate.tag.key` | `keyword` | Tag key associated with the incident. |
...
|`fortinet.fortigate.tzone` | `keyword` | Timezone offset value reported by Fortigate. |
|`fortinet.fortigate.ui` | `keyword` | User interface information reported by Fortigate. |
|`fortinet.fortigate.user.source` | `keyword` | The source of the username |
|`fortinet.fortigate.username` | `keyword` | Username value reported by the incident detection payload. |
...
|`fortinet.fortigate.xauth.group` | `keyword` | VPN XAuth group value reported by Fortigate. |
|`fortinet.fortigate.xauth.user` | `keyword` | VPN XAuth user value reported by Fortigate. |
