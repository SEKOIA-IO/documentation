
### Event Categories


The following table lists the data source offered by this integration.

| Data Source | Description                          |
| ----------- | ------------------------------------ |
| `Authentication logs` | WALLIX provides access logs |





In details, the following table denotes the type of events produced by this integration.

| Name | Values |
| ---- | ------ |
| Kind | `` |
| Category | `session` |
| Type | `end`, `info` |




### Transformed Events Samples after Ingestion

This section demonstrates how the raw logs will be transformed by our parsers. It shows the extracted fields that will be available for use in the [built-in detection rules](/xdr/features/detect/rules_catalog.md) and hunting activities in the [events page](/xdr/features/investigate/events.md). Understanding these transformations is essential for analysts to create effective detection mechanisms with [custom detection rules](/xdr/features/detect/sigma.md) and to leverage the full potential of the collected data.

=== "cron.json"

    ```json
	
    {
        "message": "pam_unix(cron:session): session closed for user root",
        "event": {
            "provider": "cron",
            "reason": "session closed"
        },
        "related": {
            "user": [
                "root"
            ]
        },
        "user": {
            "name": "root"
        }
    }
    	
	```


=== "pam_unix.json"

    ```json
	
    {
        "message": "pam_unix(sudo:session): session closed for user user1",
        "event": {
            "provider": "sudo",
            "reason": "session closed"
        },
        "related": {
            "user": [
                "user1"
            ]
        },
        "user": {
            "name": "user1"
        }
    }
    	
	```


=== "pam_unix_02.json"

    ```json
	
    {
        "message": "pam_unix(sshd:session): session closed for user root",
        "event": {
            "provider": "sshd",
            "reason": "session closed"
        },
        "related": {
            "user": [
                "root"
            ]
        },
        "user": {
            "name": "root"
        }
    }
    	
	```


=== "pam_unix_03.json"

    ```json
	
    {
        "message": "pam_unix(sshd:session): session opened for user root(uid=0) by (uid=0)",
        "event": {
            "provider": "sshd",
            "reason": "session opened"
        },
        "related": {
            "user": [
                "root"
            ]
        },
        "user": {
            "name": "root"
        }
    }
    	
	```


=== "rdpproxy.json"

    ```json
	
    {
        "message": "2022-09-05T10:05:15+02:00 foo-bastion-bar rdpproxy 4597 - - [RDP Session] session_id=\"1830c6973a42698400505688c380\" client_ip=\"1.1.1.1\" target_ip=\"2.2.2.2\" user=\"user1\" device=\"foo-bar-baz\" service=\"RDP\" account=\"user1\" type=\"FOREGROUND_WINDOW_CHANGED\" text=\"Espace de travail - IBM Lotus Notes\" class_name=\"SWT_Window0\" command_line=\"\\\"C:/lotus/Notes852/framework/rcp/eclipse/plugins/com.ibm.rcp.base_6.2.2.20100729-1241\\\\win32\\\\x86\\\\notes2.exe\\\" --launcher.suppressErrors \\\"-nosplash\\\" \\\"-nl\\\" \\\"fr\\\" \\\"-dir\\\" \\\"ltr\\\" \\\"-NPARAMS\\\" \\\"/authenticate\\\" \\\"-RPARAMS\\\" \\\"-name\\\" \\\"IBM Lotus Notes\\\" -personality \\\"com.ibm.rcp.platform.personality\\\" -product \\\"com.ibm.rcp.personality.framework.RCPProduct:com.ibm.notes.branding.notes\\\" -data \\\"c:/Lotus/Notes852/Data/workspace\\\" -configuration \\\"c:/Lotus/Notes852/Data/workspace/.config\\\" -plugincustomization \\\"C:/lotus/Notes852/framework/rcp/plugin_customization.ini\\\" -vm \\\"C:/lotus/Notes852/framework/../jvm/bin/\\\" -startup \\\"C:/lotus/Notes852/framework/rcp/eclipse/plugins/com.ibm.rcp.base_6.2.2.20100729-1241/launcher.jar\\\" -vmargs \\\"-Djava.security.policy=C:/lotus/Notes852/framework/../java.policy\\\" \\\"-Dorg.eclipse.swt.fixCitrix=false\\\" \\\"-Dosgi.framework.extensions=com.ibm.rcp.core.logger.frameworkhook,com.ibm.rds,com.ibm.cds\\\" \\\"-Xscmx64m\\\" \\\"-Xshareclasses:name=xpdplat_.jvm,controlDir=c:/Lotus/Notes852/Data/workspace/.config/org.eclipse.osgi,groupAccess,keep,singleJVM,nonfatal\\\" \\\"-Xgcpolicy:gencon\\\" \\\"-Xjit:noResumableTrapHandler\\\" \\\"-Xmaxt0.6\\\" \\\"-Xmca8k\\\" \\\"-Xminf0.1\\\" \\\"-Xmn7m\\\" \\\"-Xms48m\\\" \\\"-Xmx256m\\\" \\\"-Xnolinenumbers\\\" \\\"-Xverify:none\\\" \\\"-Xquickstart\\\" \\\"-Xscmaxaot12m\\\" \\\"-Xtrace:none\\\" \\\"-Xzero\\\" -Drcp.home=\\\"C:\\\\lotus\\\\Notes852\\\\framework\\\" -Drcp.data=\\\"c:/Lotus/Notes852/Data/workspace\\\" -Dosgi.splashPath=\\\"platform:/base/../../shared/eclipse/plugins/com.ibm.notes.branding,platform:/base/../../shared/eclipse/plugins/com.ibm.notes.branding.nl1,platform:/base/../.shared/eclipse/plugins/com.ibm.notes.branding.nl2,platform:/base/../../shared/eclipse/plugins/com.ibm.notes.branding.nl3\\\" -Dcom.ibm.rcp.install.id=\\\"1320657906134\\\" -Drcp.install.config=\\\"user\\\" -Declipse.registry.nulltoken=\\\"true\\\" -Dautopd.logfile.generations=\\\"3\\\" -Dorg.apache.xerces.xni.parser.XMLParserConfiguration=\\\"org.apache.xerces.parsers.XIncludeAwareParserConfiguration\\\" -Dcom.ibm.pvc.webcontainer.http.address=\\\"localhost\\\" -Dosgi.nl.user=\\\"true\\\" -Dautopd.instance.area=\\\"c:/Lotus/Notes852/Data/workspace/autopd\\\" -Dorg.eclipse.swt.browser.XULRunnerPath=\\\"C:/lotus/Notes852/framework/rcp/eclipse/plugins/com.ibm.rcp.xulrunner.runtime.win32.x86_6.2.2.20100729-1241/xulrunner\\\" -Djava.util.logging.config.class=\\\"com.ibm.rcp.core.internal.logger.boot.LoggerConfig\\\" -Dcom.ibm.pvc.webcontainer.port=\\\"0,59449\\\" -Dcom.ibm.pvc.webcontainer.vhost.configfile=\\\"C:/lotus/Notes852/framework/shared/eclipse/plugins/com.ibm.collaboration.realtime.webapi_8.0.2.20100802-0849/virtualhost.properties\\\" -Dderby.stream.error.file=\\\"c:/Lotus/Notes852/Data/workspace/logs/derby.log\\\" -Djava.security.properties=\\\"file:C:/lotus/Notes852/framework/rcp/eclipse/plugins/com.ibm.rcp.base_6.2.2.20100729-1241/rcp.security.properties\\\" -Djava.protocol.handler.pkgs=\\\"com.ibm.net.ssl.www.protocol\\\" -Dosgi.hook.configurators.exclude=\\\"org.eclipse.core.runtime.internal.adaptor.EclipseLogHook\\\" -Drcp.osgi.install.area=\\\"C:\\\\lotus\\\\Notes852\\\\framework\\\\rcp\\\\eclipse\\\" -Xbootclasspath/a:\\\"C:/lotus/Notes852/framework/shared/eclipse/plugins/com.ibm.collaboration.realtime.stjavatk_8.0.2.20100802-0849/sslite140-v3.16.jar;C:/lotus/Notes852/framework/rceclipse/plugins/com.ibm.rcp.base_6.2.2.20100729-1241/rcpbootcp.jar\\\"\"\n\n",
        "event": {
            "action": "FOREGROUND_WINDOW_CHANGED"
        },
        "destination": {
            "address": "2.2.2.2",
            "ip": "2.2.2.2"
        },
        "related": {
            "ip": [
                "1.1.1.1",
                "2.2.2.2"
            ],
            "user": [
                "user1"
            ]
        },
        "service": {
            "name": "RDP"
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "type": "FOREGROUND_WINDOW_CHANGED"
        }
    }
    	
	```


=== "rdpproxy2.json"

    ```json
	
    {
        "message": "2022-09-05T10:05:15+02:00 foo-bastion-bar rdpproxy 13297 - - [RDP Session] session_id=\"1830c403be7caf0c00505688c380\" client_ip=\"1.1.1.1\" target_ip=\"2.2.2.2\" user=\"user1\" device=\"foo-bastion-bar\" service=\"RDP\" account=\"user1@corp.net\" type=\"FOREGROUND_WINDOW_CHANGED\" text=\"Remote Desktop Manager Free [FOO-BAR-P01]\" class_name=\"WindowsForms10.Window.8.app.0.13965fa_r6_ad1\" command_line=\"\\\"C:\\\\Program Files (x86)\\\\Devolutions\\\\Remote Desktop Manager Free\\\\RemoteDesktopManagerFree.exe\\\" \"\n\n",
        "event": {
            "action": "FOREGROUND_WINDOW_CHANGED"
        },
        "destination": {
            "address": "2.2.2.2",
            "ip": "2.2.2.2"
        },
        "related": {
            "ip": [
                "1.1.1.1",
                "2.2.2.2"
            ],
            "user": [
                "user1"
            ]
        },
        "service": {
            "name": "RDP"
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "type": "FOREGROUND_WINDOW_CHANGED"
        }
    }
    	
	```


=== "sshprox.json"

    ```json
	
    {
        "message": "[sshproxy] psid=\"161607370130601\" type=\"INCOMING_CONNECTION\" src_ip=\"10.17.86.250\" src_port=\"53344\"",
        "event": {
            "action": "INCOMING_CONNECTION",
            "provider": "sshproxy"
        },
        "related": {
            "ip": [
                "10.17.86.250"
            ]
        },
        "source": {
            "address": "10.17.86.250",
            "ip": "10.17.86.250",
            "port": 53344
        },
        "wallix": {
            "type": "INCOMING_CONNECTION"
        }
    }
    	
	```


=== "sshproxy.json"

    ```json
	
    {
        "message": "sshproxy: [SSH Session] session_id=\"168bd3b417f437ae005056b60af6\" client_ip=\"10.10.43.84\" target_ip=\"10.10.47.53\" user=\"user01\" device=\"10.10.47.53\" service=\"ssh\" account=\"root\" type=\"SESSION_DISCONNECTION\" duration=\"0:00:05\"",
        "event": {
            "action": "SESSION_DISCONNECTION"
        },
        "destination": {
            "address": "10.10.47.53",
            "ip": "10.10.47.53"
        },
        "host": {
            "ip": "10.10.47.53"
        },
        "related": {
            "ip": [
                "10.10.43.84",
                "10.10.47.53"
            ],
            "user": [
                "user01"
            ]
        },
        "service": {
            "name": "ssh"
        },
        "source": {
            "address": "10.10.43.84",
            "ip": "10.10.43.84"
        },
        "user": {
            "name": "user01"
        },
        "wallix": {
            "type": "SESSION_DISCONNECTION"
        }
    }
    	
	```


=== "sshsession.json"

    ```json
	
    {
        "message": "[SSH Session] session_id=\"182f9642c81320eb0050568e16d9\" client_ip=\"1.1.1.1\" target_ip=\"1.1.1.1\" user=\"username123@corp.net\" device=\"1.1.1.1\" service=\"SSH\" account=\"username123\" type=\"SESSION_DISCONNECTION\" duration=\"0:59:57\"",
        "event": {
            "action": "SESSION_DISCONNECTION",
            "provider": "SSH Session"
        },
        "destination": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "host": {
            "ip": "1.1.1.1"
        },
        "related": {
            "ip": [
                "1.1.1.1"
            ],
            "user": [
                "username123@corp.net"
            ]
        },
        "service": {
            "name": "SSH"
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "username123@corp.net"
        },
        "wallix": {
            "type": "SESSION_DISCONNECTION"
        }
    }
    	
	```


=== "sshsession2.json"

    ```json
	
    {
        "message": "[SSH Session] session_id=\"1830cbf7a55a11dd005056b01296\" client_ip=\"1.1.1.1\" target_ip=\"ip-foo-bar-baz.corp.net\" user=\"user1\" device=\"DEVICE-FOO\" service=\"SSH\" account=\"username\" type=\"SESSION_ESTABLISHED_SUCCESSFULLY\"",
        "event": {
            "action": "SESSION_ESTABLISHED_SUCCESSFULLY",
            "provider": "SSH Session"
        },
        "destination": {
            "address": "ip-foo-bar-baz.corp.net",
            "domain": "ip-foo-bar-baz.corp.net",
            "registered_domain": "corp.net",
            "subdomain": "ip-foo-bar-baz",
            "top_level_domain": "net"
        },
        "related": {
            "hosts": [
                "ip-foo-bar-baz.corp.net"
            ],
            "ip": [
                "1.1.1.1"
            ],
            "user": [
                "user1"
            ]
        },
        "service": {
            "name": "SSH"
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "type": "SESSION_ESTABLISHED_SUCCESSFULLY"
        }
    }
    	
	```


=== "sudo.json"

    ```json
	
    {
        "message": "root : TTY=unknown ; PWD=/root ; USER=user1 ; COMMAND=/opt/wab/bin/WABCleanApprovals close",
        "event": {
            "provider": "sudo"
        },
        "process": {
            "command_line": "/opt/wab/bin/WABCleanApprovals close",
            "working_directory": "/root"
        },
        "related": {
            "user": [
                "user1"
            ]
        },
        "user": {
            "name": "user1"
        },
        "wallix": {}
    }
    	
	```


=== "test_completed_event_event.json"

    ```json
	
    {
        "message": "2025-04-23T17:29:22+02:00 test rdpproxy 11292 - - [RDP Session] session_id=\"19662eb3c2c67400005000000000\" client_ip=\"192.0.2.1\" target_ip=\"192.0.2.1\" user=\"test@test.fr\" device=\"example.com\" service=\"RDP\" account=\"test@test.fr\" type=\"COMPLETED_PROCESS\" command_line=\"C:\\\\Windows\\\\System32\\\\testBroker.exe -Embedding\"",
        "event": {
            "action": "COMPLETED_PROCESS"
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "host": {
            "name": "example.com"
        },
        "process": {
            "command_line": "C:\\Windows\\System32\\testBroker.exe -Embedding"
        },
        "related": {
            "ip": [
                "192.0.2.1"
            ],
            "user": [
                "test@test.fr"
            ]
        },
        "service": {
            "name": "RDP"
        },
        "source": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "user": {
            "name": "test@test.fr"
        },
        "wallix": {
            "type": "COMPLETED_PROCESS"
        }
    }
    	
	```


=== "test_event_1.json"

    ```json
	
    {
        "message": "pam_unix(systemd-user:session): session opened for user user1 by (uid=0)",
        "event": {
            "reason": "session opened"
        },
        "related": {
            "user": [
                "user1"
            ]
        },
        "user": {
            "name": "user1"
        }
    }
    	
	```


=== "test_event_2.json"

    ```json
	
    {
        "message": "pam_unix(systemd-user:session): session closed for user user1",
        "event": {
            "reason": "session closed"
        },
        "related": {
            "user": [
                "user1"
            ]
        },
        "user": {
            "name": "user1"
        }
    }
    	
	```


=== "test_new_process_event.json"

    ```json
	
    {
        "message": "2025-04-23T16:24:52+02:00 test01 rdpproxy 32019 - - [RDP Session] session_id=\"19662b5f089766d40050000400000\" client_ip=\"1.1.1.1\" target_ip=\"192.0.2.1\" user=\"test@test.fr\" device=\"example.com\" service=\"RDP\" account=\"test@test.fr\" type=\"NEW_PROCESS\" command_line=\"\\\"C:\\\\Windows\\\\system32\\\\test_script.exe\\\" -ServerName:serverTest\"",
        "event": {
            "action": "NEW_PROCESS"
        },
        "destination": {
            "address": "192.0.2.1",
            "ip": "192.0.2.1"
        },
        "host": {
            "name": "example.com"
        },
        "process": {
            "command_line": "\\\"C:\\Windows\\system32\\test_script.exe\\\" -ServerName:serverTest"
        },
        "related": {
            "ip": [
                "1.1.1.1",
                "192.0.2.1"
            ],
            "user": [
                "test@test.fr"
            ]
        },
        "service": {
            "name": "RDP"
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "test@test.fr"
        },
        "wallix": {
            "type": "NEW_PROCESS"
        }
    }
    	
	```


=== "test_rdp_session.json"

    ```json
	
    {
        "message": "[RDP Session] session_id=\"57a6694d877c413ba502946a03461dd2\" client_ip=\"1.2.3.4\" target_ip=\"5.6.7.8\" user=\"john.doe@example.org\" device=\"HOST0102\" service=\"RDP\" account=\"u10293@platform.example.org\" type=\"KBD_INPUT\" data=\"cusi/<backspace>/<backspace>si/<backspace>/<backspace>is\"\n",
        "event": {
            "action": "KBD_INPUT"
        },
        "destination": {
            "address": "5.6.7.8",
            "ip": "5.6.7.8"
        },
        "related": {
            "ip": [
                "1.2.3.4",
                "5.6.7.8"
            ],
            "user": [
                "john.doe@example.org"
            ]
        },
        "service": {
            "name": "RDP"
        },
        "source": {
            "address": "1.2.3.4",
            "ip": "1.2.3.4"
        },
        "user": {
            "name": "john.doe@example.org"
        },
        "wallix": {
            "data": "cusi/<backspace>/<backspace>si/<backspace>/<backspace>is",
            "type": "KBD_INPUT"
        }
    }
    	
	```


=== "test_session_integrity_1.json"

    ```json
	
    {
        "message": "[sessionintegrity] session_uid=\"1830c403be7caf0c00505688c380\" status=\"failed\" type=\"SSH_SHELL_SESSION\" user=\"user1\" target=\"domain@local@target01.corp.net:SSH_1\" begin=\"2022-08-19 11:31:17\" end=\"2022-08-19 11:32:50\" files=[/var/wab/remote/recorded/ssh/2022-08-19/182b5714b466cba10050568e16d9,user1,domain@target01.corp.net,20220819-113117,foo-bastion-bar.corp.net,1805.ttyrec]",
        "event": {
            "action": "SSH_SHELL_SESSION",
            "category": [
                "session"
            ],
            "dataset": "session_integrity",
            "end": "2022-08-19T11:32:50Z",
            "outcome": "failure",
            "start": "2022-08-19T11:31:17Z",
            "type": [
                "info"
            ]
        },
        "file": {
            "directory": "/var/wab/remote/recorded/ssh/2022-08-19/182b5714b466cba10050568e16d9/user1/domain@target01.corp.net/20220819-113117/foo-bastion-bar.corp.net",
            "extension": "ttyrec",
            "name": "1805.ttyrec",
            "path": "/var/wab/remote/recorded/ssh/2022-08-19/182b5714b466cba10050568e16d9/user1/domain@target01.corp.net/20220819-113117/foo-bastion-bar.corp.net/1805.ttyrec"
        },
        "related": {
            "user": [
                "user1"
            ]
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "type": "SSH_SHELL_SESSION"
        }
    }
    	
	```


=== "test_session_integrity_2.json"

    ```json
	
    {
        "message": "[sessionintegrity] session_uid=\"19832205e067b1b9005056b11d7f\" status=\"failed\" type=\"RDP\" user=\"testuser@example.org\" target=\"testuser@database01.example.com:RDP\" begin=\"2025-07-22 14:33:58\" end=\"2025-07-22 15:10:59\" file=[/var/wab/remote/recorded/rdp/2025-07-22/19832205e067b1b9005056b11d7f,20250722-143358,bastion-prod-01,4055-000002.wrm]",
        "event": {
            "action": "RDP",
            "category": [
                "session"
            ],
            "dataset": "session_integrity",
            "end": "2025-07-22T15:10:59Z",
            "outcome": "failure",
            "start": "2025-07-22T14:33:58Z",
            "type": [
                "info"
            ]
        },
        "file": {
            "directory": "/var/wab/remote/recorded/rdp/2025-07-22/19832205e067b1b9005056b11d7f/20250722-143358/bastion-prod-01",
            "extension": "wrm",
            "name": "4055-000002.wrm",
            "path": "/var/wab/remote/recorded/rdp/2025-07-22/19832205e067b1b9005056b11d7f/20250722-143358/bastion-prod-01/4055-000002.wrm"
        },
        "related": {
            "user": [
                "testuser@example.org"
            ]
        },
        "user": {
            "name": "testuser@example.org"
        },
        "wallix": {
            "type": "RDP"
        }
    }
    	
	```


=== "test_sftp_event.json"

    ```json
	
    {
        "message": "2025-06-16T17:28:09+02:00 wabp01 sshproxy 2752422 - - [SSH Session] session_id=\"19777cd1cad10fec00505694f38a\" client_ip=\"1.2.3.4\" target_ip=\"example.com\" user=\"username\" device=\"PC01\" service=\"SSH\" account=\"gtk\" type=\"SFTP_EVENT\" status=\"Success\" data=\"setstat '/gtk/tmp/graphtalk-admin-console.war' uid='0' gid='0'\"",
        "event": {
            "action": "SFTP_EVENT"
        },
        "destination": {
            "address": "example.com",
            "domain": "example.com",
            "registered_domain": "example.com",
            "top_level_domain": "com"
        },
        "related": {
            "hosts": [
                "example.com"
            ],
            "ip": [
                "1.2.3.4"
            ],
            "user": [
                "username"
            ]
        },
        "service": {
            "name": "SSH"
        },
        "source": {
            "address": "1.2.3.4",
            "ip": "1.2.3.4"
        },
        "user": {
            "name": "username"
        },
        "wallix": {
            "data": "setstat '/gtk/tmp/graphtalk-admin-console.war' uid='0' gid='0'",
            "type": "SFTP_EVENT"
        }
    }
    	
	```


=== "test_vault_event.json"

    ```json
	
    {
        "message": "2025-04-23T16:18:12+02:00 test02 WAB(root) 17470 - - [Vault Activity] type=\"Vault\" action=\"checkin\" user=\"test@test.fr\" account=\"user1\" vault=\"local\" session=\"True\" result=\"Checkin successful\"",
        "event": {
            "action": "Vault"
        },
        "destination": {
            "user": {
                "name": "user1"
            }
        },
        "related": {
            "user": [
                "test@test.fr",
                "user1"
            ]
        },
        "user": {
            "name": "test@test.fr"
        },
        "wallix": {
            "action": "checkin",
            "type": "Vault"
        }
    }
    	
	```


=== "wabaudit_action_add_type_ConnectionPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"ConnectionPolicy\" object=\"QA_CONNECTION_POLICY_SSH_AGENT_FORWARDING\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"cn [QA_CONNECTION_POLICY_SSH_AGENT_FORWARDING], protocol [SSH], services [], methods [PASSWORD_VAULT, PUBKEY_VAULT, PUBKEY_AGENT_FORWARDING and 1 other(s)], Data [server_pubkey[server_pubkey_check]: '1', server_pubkey[server_pubkey_create_message]: '1', server_pubkey[server_access_allowed_message]: '0', server_pubkey[server_pubkey_success_message]: '0', server_pubkey[server_pubkey_failure_message]: '1', server_pubkey[server_pubkey_store]: 'True', trace[log_all_kbd]: 'False', startup_scenario[ask_startup]: 'False', startup_scenario[show_output]: 'True', startup_scenario[enable]: 'False', startup_scenario[timeout]: '10', startup_scenario[scenario]: '', general[transformation_rule]: '', session[inactivity_timeout]: '0', session[allow_multi_channels]: 'False', algorithms[kex_algos]: '', algorithms[compression_algos]: '', algorithms[cipher_algos]: '', algorithms[integrity_algos]: '']\"",
        "event": {
            "action": "ConnectionPolicy",
            "provider": "wabengine",
            "reason": "cn [QA_CONNECTION_POLICY_SSH_AGENT_FORWARDING], protocol [SSH], services [], methods [PASSWORD_VAULT, PUBKEY_VAULT, PUBKEY_AGENT_FORWARDING and 1 other(s)], Data [server_pubkey[server_pubkey_check]: '1', server_pubkey[server_pubkey_create_message]: '1', server_pubkey[server_access_allowed_message]: '0', server_pubkey[server_pubkey_success_message]: '0', server_pubkey[server_pubkey_failure_message]: '1', server_pubkey[server_pubkey_store]: 'True', trace[log_all_kbd]: 'False', startup_scenario[ask_startup]: 'False', startup_scenario[show_output]: 'True', startup_scenario[enable]: 'False', startup_scenario[timeout]: '10', startup_scenario[scenario]: '', general[transformation_rule]: '', session[inactivity_timeout]: '0', session[allow_multi_channels]: 'False', algorithms[kex_algos]: '', algorithms[compression_algos]: '', algorithms[cipher_algos]: '', algorithms[integrity_algos]: '']",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_CONNECTION_POLICY_SSH_AGENT_FORWARDING",
            "type": "ConnectionPolicy"
        }
    }
    	
	```


=== "wabaudit_action_add_type_CredChgInfo.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"CredChgInfo\" object=\"local1/None\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"service_name ['None' to 'XE'], host ['None' to 'my.db.hostname'], port ['None' to '1234']\"",
        "event": {
            "action": "CredChgInfo",
            "provider": "wabengine",
            "reason": "service_name ['None' to 'XE'], host ['None' to 'my.db.hostname'], port ['None' to '1234']",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "local1/None",
            "type": "CredChgInfo"
        }
    }
    	
	```


=== "wabaudit_action_add_type_CredChgPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"CredChgPolicy\" object=\"QA_PASSWORD_CHANGE_POLICY\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"pwdLength [8], specialChars [1], changePeriod []\"",
        "event": {
            "action": "CredChgPolicy",
            "provider": "wabengine",
            "reason": "pwdLength [8], specialChars [1], changePeriod []",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_PASSWORD_CHANGE_POLICY",
            "type": "CredChgPolicy"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Globaldomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Globaldomain\" object=\"QA_DOMAIN_SIMPLE\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"cn [QA_DOMAIN_SIMPLE], name [QA_DOMAIN_SIMPLE]\"",
        "event": {
            "action": "Globaldomain",
            "provider": "wabengine",
            "reason": "cn [QA_DOMAIN_SIMPLE], name [QA_DOMAIN_SIMPLE]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DOMAIN_SIMPLE",
            "type": "Globaldomain"
        }
    }
    	
	```


=== "wabaudit_action_add_type_LdapMapping.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"LdapMapping\" object=\"<QA_DOMAIN_1, OU=Group> in user_group_154954913825 GROUP\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"ldapGroup [OU=Group], domain [QA_DOMAIN_1], group [user_group_154954913825]\"",
        "event": {
            "action": "LdapMapping",
            "provider": "wabengine",
            "reason": "ldapGroup [OU=Group], domain [QA_DOMAIN_1], group [user_group_154954913825]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "<QA_DOMAIN_1, OU=Group> in user_group_154954913825 GROUP",
            "type": "LdapMapping"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Ldapdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Ldapdomain\" object=\"QA_DOMAIN_1\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"description [], ldapDomain [domain1.qa], defaultLanguage [en], defaultEmailDomain [wallix], groupAttribute [memberOf], snAttribute [displayName], emailAttribute [mail], languageAttribute [preferredLanguage], isDefaultDomain [True]\"",
        "event": {
            "action": "Ldapdomain",
            "provider": "wabengine",
            "reason": "description [], ldapDomain [domain1.qa], defaultLanguage [en], defaultEmailDomain [wallix], groupAttribute [memberOf], snAttribute [displayName], emailAttribute [mail], languageAttribute [preferredLanguage], isDefaultDomain [True]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DOMAIN_1",
            "type": "Ldapdomain"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Localdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Localdomain\" object=\"local\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"cn [local], device [QA_DEVICE_SSH_SHELL_SESSION]\"",
        "event": {
            "action": "Localdomain",
            "provider": "wabengine",
            "reason": "cn [local], device [QA_DEVICE_SSH_SHELL_SESSION]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "local",
            "type": "Localdomain"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Notification.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Notification\" object=\"notification_154955208543\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"dest [qa-notify@wallix.com], flag [0], isNotificationEnable [True], type [EMAIL]\"",
        "event": {
            "action": "Notification",
            "provider": "wabengine",
            "reason": "dest [qa-notify@wallix.com], flag [0], isNotificationEnable [True], type [EMAIL]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "notification_154955208543",
            "type": "Notification"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Period.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Period\" object=\"<2030-01-01 to 2099-12-31 , 00:00:00 to 23:59:00, 127>\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"startDate [2030-01-01], endDate [2099-12-31], startTime [00:00:00], endTime [23:59:00], weekmask [127]\"",
        "event": {
            "action": "Period",
            "provider": "wabengine",
            "reason": "startDate [2030-01-01], endDate [2099-12-31], startTime [00:00:00], endTime [23:59:00], weekmask [127]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "<2030-01-01 to 2099-12-31 , 00:00:00 to 23:59:00, 127>",
            "type": "Period"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Profil.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Profile\" object=\"QA_PROFILE_IP_FORBIDDEN\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"ip_limitation [1.1.1.1], habilitationFlag [1], groups_limitation [], groups_member []\"",
        "event": {
            "action": "Profile",
            "provider": "wabengine",
            "reason": "ip_limitation [1.1.1.1], habilitationFlag [1], groups_limitation [], groups_member []",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_PROFILE_IP_FORBIDDEN",
            "type": "Profile"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Restriction.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Restriction\" object=\"<kill, Kill.+Softly, SSH_SHELL_SESSION> in GROUP QA_USER_GROUP_UNIX_KILL\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"action [kill], data [Kill.+Softly], groups [QA_USER_GROUP_UNIX_KILL], subprotocol [SSH_SHELL_SESSION]\"",
        "event": {
            "action": "Restriction",
            "provider": "wabengine",
            "reason": "action [kill], data [Kill.+Softly], groups [QA_USER_GROUP_UNIX_KILL], subprotocol [SSH_SHELL_SESSION]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "<kill, Kill.+Softly, SSH_SHELL_SESSION> in GROUP QA_USER_GROUP_UNIX_KILL",
            "type": "Restriction"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Service.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Service\" object=\"QA_DEVICE_SSH_SHELL_SESSION:SSH\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"protocol [SSH], port [22], subprotocols [SSH_SHELL_SESSION], connectionPolicy [SSH]\"",
        "event": {
            "action": "Service",
            "provider": "wabengine",
            "reason": "protocol [SSH], port [22], subprotocols [SSH_SHELL_SESSION], connectionPolicy [SSH]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DEVICE_SSH_SHELL_SESSION:SSH",
            "type": "Service"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Targetgroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Targetgroup\" object=\"QA_DEVICE_GROUP_UNIX\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"Users [], Targets [__WIL__@am_il_domain@QA_DEVICE_TELNET:TELNET, __WAM__@am_il_domain@QA_DEVICE_SSH_SCP_DOWN:SSH, pubkey_account_without_password@local@QA_DEVICE_SSH_FORWARDING:SSH and 35 other(s)], Profiles_limit [], Timeframes [allthetime]\"",
        "event": {
            "action": "Targetgroup",
            "provider": "wabengine",
            "reason": "Users [], Targets [__WIL__@am_il_domain@QA_DEVICE_TELNET:TELNET, __WAM__@am_il_domain@QA_DEVICE_SSH_SCP_DOWN:SSH, pubkey_account_without_password@local@QA_DEVICE_SSH_FORWARDING:SSH and 35 other(s)], Profiles_limit [], Timeframes [allthetime]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DEVICE_GROUP_UNIX",
            "type": "Targetgroup"
        }
    }
    	
	```


=== "wabaudit_action_add_type_TimeFrame.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"TimeFrame\" object=\"timeframe_154954856399\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"description [], isOvertimable [False]\"",
        "event": {
            "action": "TimeFrame",
            "provider": "wabengine",
            "reason": "description [], isOvertimable [False]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "timeframe_154954856399",
            "type": "TimeFrame"
        }
    }
    	
	```


=== "wabaudit_action_add_type_User.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"User\" object=\"QA_USER_IP_FORBIDDEN\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"email [qa-notify@wallix.com], preferredLanguage [en], host [1.1.1.1], profile [user], groups [QA_USER_GROUP_UNIX], forceChangePwd [False], userPassword [********], userauths [local]\"",
        "event": {
            "action": "User",
            "provider": "wabengine",
            "reason": "email [qa-notify@wallix.com], preferredLanguage [en], host [1.1.1.1], profile [user], groups [QA_USER_GROUP_UNIX], forceChangePwd [False], userPassword [********], userauths [local]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_USER_IP_FORBIDDEN",
            "type": "User"
        }
    }
    	
	```


=== "wabaudit_action_add_type_UserAuth.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"UserAuth\" object=\"QA_USER_AUTH_KERBEROS\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"wabAuthType [KERBEROS], description [], port [88], host [10.10.45.148], kerDomControler [QA.IFR.LAN]\"",
        "event": {
            "action": "UserAuth",
            "provider": "wabengine",
            "reason": "wabAuthType [KERBEROS], description [], port [88], host [10.10.45.148], kerDomControler [QA.IFR.LAN]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_USER_AUTH_KERBEROS",
            "type": "UserAuth"
        }
    }
    	
	```


=== "wabaudit_action_add_type_Usergroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Usergroup\" object=\"QA_USER_GROUP_UNIX\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"Users [], Profiles_limit [], Timeframes [allthetime]\"",
        "event": {
            "action": "Usergroup",
            "provider": "wabengine",
            "reason": "Users [], Profiles_limit [], Timeframes [allthetime]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_USER_GROUP_UNIX",
            "type": "Usergroup"
        }
    }
    	
	```


=== "wabaudit_action_add_type_X509Parameters.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"X509 Parameters\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"CRL [url fetched hourly]\"",
        "event": {
            "action": "X509 Parameters",
            "provider": "wabengine",
            "reason": "CRL [url fetched hourly]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "type": "X509 Parameters"
        }
    }
    	
	```


=== "wabaudit_action_add_type_account.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Account\" object=\"account_with_approval@QA_DOMAIN_SIMPLE\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"name [account_with_approval], login [account_with_approval], autoChangePassword [True], autoChangeSSHKey [True], isExternalVault [False]\"",
        "event": {
            "action": "Account",
            "provider": "wabengine",
            "reason": "name [account_with_approval], login [account_with_approval], autoChangePassword [True], autoChangeSSHKey [True], isExternalVault [False]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "account_with_approval@QA_DOMAIN_SIMPLE",
            "type": "Account"
        }
    }
    	
	```


=== "wabaudit_action_add_type_answer.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"accountactivity\" object=\"168c1c48f141e911005056b60af6\" user=\"admin\" client_ip=\"10.10.43.84\" infos=",
        "event": {
            "action": "accountactivity",
            "provider": "wabengine",
            "type": [
                "access"
            ]
        },
        "related": {
            "ip": [
                "10.10.43.84"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.43.84",
            "ip": "10.10.43.84"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "list",
            "object": "168c1c48f141e911005056b60af6",
            "type": "accountactivity"
        }
    }
    	
	```


=== "wabaudit_action_add_type_apikey.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Apikey\" object=\"apikey_154954880399\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"cn [apikey_154954880399], apikey [********], ipLimitation []\"",
        "event": {
            "action": "Apikey",
            "provider": "wabengine",
            "reason": "cn [apikey_154954880399], apikey [********], ipLimitation []",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "apikey_154954880399",
            "type": "Apikey"
        }
    }
    	
	```


=== "wabaudit_action_add_type_application.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Application\" object=\"QA_APP_DUMMY\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"target [account@local@QA_DEVICE_DUMMY_WIN:RDP]\"",
        "event": {
            "action": "Application",
            "provider": "wabengine",
            "reason": "target [account@local@QA_DEVICE_DUMMY_WIN:RDP]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_APP_DUMMY",
            "type": "Application"
        }
    }
    	
	```


=== "wabaudit_action_add_type_apppath.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Apppath\" object=\"account@local@QA_DEVICE_DUMMY_WIN:RDP[<C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe>:<C:\\>]\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"program [C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe], workingdir [C:\\]\"",
        "event": {
            "action": "Apppath",
            "provider": "wabengine",
            "reason": "program [C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe], workingdir [C:\\]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "account@local@QA_DEVICE_DUMMY_WIN:RDP[<C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe>:<C:\\>]",
            "type": "Apppath"
        }
    }
    	
	```


=== "wabaudit_action_add_type_approval.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Approval\" object=\"<Approval(uid=\\'168c849f0378d7f4005056b69255\\', status=3, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"status [3], begin [2019-02-07 15:08:00], creation [2019-02-07 15:08:35.382824], duration [600], end [2019-02-07 15:18:00], username [user1], targetname [user_1@local@QA_DEVICE_WITH_APPROVAL_OPTIONAL_COMMENT_AND_TICKET:SSH], quorum [1], email [qa-notify@wallix.com], language [en]\"",
        "event": {
            "action": "Approval",
            "provider": "wabengine",
            "reason": "status [3], begin [2019-02-07 15:08:00], creation [2019-02-07 15:08:35.382824], duration [600], end [2019-02-07 15:18:00], username [user1], targetname [user_1@local@QA_DEVICE_WITH_APPROVAL_OPTIONAL_COMMENT_AND_TICKET:SSH], quorum [1], email [qa-notify@wallix.com], language [en]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "<Approval(uid=\\'168c849f0378d7f4005056b69255\\', status=3, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n",
            "type": "Approval"
        }
    }
    	
	```


=== "wabaudit_action_add_type_authorization.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Authorization\" object=\"QA_USER_GROUP_UNIX:QA_DEVICE_GROUP_UNIX\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"cn [unix_group], targetGroupIdentifier [QA_DEVICE_GROUP_UNIX], isRecorded [True], isCritical [False], userAccess [False], proxyAccess [True], subprotocols [SSH_SHELL_SESSION, SSH_REMOTE_COMMAND, SSH_SCP_UP and 7 other(s)], approvalRequired [False], hasComment [False], mandatoryComment [False], hasTicket [False], mandatoryTicket [False], activeQuorum [0], inactiveQuorum [0]\"",
        "event": {
            "action": "Authorization",
            "provider": "wabengine",
            "reason": "cn [unix_group], targetGroupIdentifier [QA_DEVICE_GROUP_UNIX], isRecorded [True], isCritical [False], userAccess [False], proxyAccess [True], subprotocols [SSH_SHELL_SESSION, SSH_REMOTE_COMMAND, SSH_SCP_UP and 7 other(s)], approvalRequired [False], hasComment [False], mandatoryComment [False], hasTicket [False], mandatoryTicket [False], activeQuorum [0], inactiveQuorum [0]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_USER_GROUP_UNIX:QA_DEVICE_GROUP_UNIX",
            "type": "Authorization"
        }
    }
    	
	```


=== "wabaudit_action_add_type_checkoutpolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"CheckoutPolicy\" object=\"QA_CHECKOUT_POLICY_LOCK\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"enableLock [True], duration [600], extension [0], maxDuration [600], checkinChange [0]\"",
        "event": {
            "action": "CheckoutPolicy",
            "provider": "wabengine",
            "reason": "enableLock [True], duration [600], extension [0], maxDuration [600], checkinChange [0]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_CHECKOUT_POLICY_LOCK",
            "type": "CheckoutPolicy"
        }
    }
    	
	```


=== "wabaudit_action_add_type_cluster.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Cluster\" object=\"cluster_154954837225\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"member_targets [account_154954837122@local1@device_154954837021:rdp, account_154954837224@local1@device_154954837123:rdp]\"",
        "event": {
            "action": "Cluster",
            "provider": "wabengine",
            "reason": "member_targets [account_154954837122@local1@device_154954837021:rdp, account_154954837224@local1@device_154954837123:rdp]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "add",
            "object": "cluster_154954837225",
            "type": "Cluster"
        }
    }
    	
	```


=== "wabaudit_action_add_type_device.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Device\" object=\"QA_DEVICE_SSH_SHELL_SESSION\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"Host [10.10.45.148], Alias [QA_DEVICE_SSH_SHELL_SESSION_ALIAS]\"",
        "event": {
            "action": "Device",
            "provider": "wabengine",
            "reason": "Host [10.10.45.148], Alias [QA_DEVICE_SSH_SHELL_SESSION_ALIAS]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DEVICE_SSH_SHELL_SESSION",
            "type": "Device"
        }
    }
    	
	```


=== "wabaudit_action_backup_type_backup_restore.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"backup\" type=\"Backup/Restore\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' saved]\"",
        "event": {
            "action": "Backup/Restore",
            "category": [
                "database"
            ],
            "provider": "wabengine",
            "reason": "Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' saved]"
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "backup",
            "type": "Backup/Restore"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_ConnectionPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"ConnectionPolicy\" object=\"connection_policy_154954884812\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "ConnectionPolicy",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "connection_policy_154954884812",
            "type": "ConnectionPolicy"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_CredChgInfo.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"CredChgInfo\" object=\"<CredChgInfo(uid=\\'168c849848928a52005056b69255\\')>\\n\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CredChgInfo",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "<CredChgInfo(uid=\\'168c849848928a52005056b69255\\')>\\n",
            "type": "CredChgInfo"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_CredChgPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"CredChgPolicy\" object=\"password_change_policy_name_154954918141\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CredChgPolicy",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "password_change_policy_name_154954918141",
            "type": "CredChgPolicy"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Globaldomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Globaldomain\" object=\"global_domain_154954904181\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Globaldomain",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "global_domain_154954904181",
            "type": "Globaldomain"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_LdapMapping.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"LdapMapping\" object=\"<QA_DOMAIN_1, OU=Group> in user_group_154954913825 GROUP\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "LdapMapping",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "<QA_DOMAIN_1, OU=Group> in user_group_154954913825 GROUP",
            "type": "LdapMapping"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Ldapdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Ldapdomain\" object=\"domain_154955334782\" user=\"admin\" client_ip=\"192.168.122.1\" infos=\"\"",
        "event": {
            "action": "Ldapdomain",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "192.168.122.1"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.122.1",
            "ip": "192.168.122.1"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "delete",
            "object": "domain_154955334782",
            "type": "Ldapdomain"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Localdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Localdomain\" object=\"local1\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Localdomain",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "local1",
            "type": "Localdomain"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Notification.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Notification\" object=\"notification_154955204621\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Notification",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "notification_154955204621",
            "type": "Notification"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Period.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Period\" object=\"<2010-01-01 to 2020-01-01 , 09:30:00 to 18:30:00, 124>\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Period",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "<2010-01-01 to 2020-01-01 , 09:30:00 to 18:30:00, 124>",
            "type": "Period"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Profil.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Profile\" object=\"profile_154954924847\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Profile",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "profile_154954924847",
            "type": "Profile"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Service.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Service\" object=\"device_154954928856:ssh\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Service",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "device_154954928856:ssh",
            "type": "Service"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Targetgroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Targetgroup\" object=\"target_group_154954938767\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Targetgroup",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "target_group_154954938767",
            "type": "Targetgroup"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_TimeFrame.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"TimeFrame\" object=\"timeframe_154954953374\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "TimeFrame",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "timeframe_154954953374",
            "type": "TimeFrame"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_User.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"User\" object=\"UNKNOWN_USER\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "User",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "UNKNOWN_USER",
            "type": "User"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_UserAuth.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"UserAuth\" object=\"auth_LDAP_154955198487\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "UserAuth",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "auth_LDAP_154955198487",
            "type": "UserAuth"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_Usergroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Usergroup\" object=\"user_group_154954962345\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Usergroup",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "user_group_154954962345",
            "type": "Usergroup"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_X509Parameters.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"X509 Parameters\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"CRL [deleted]\"",
        "event": {
            "action": "X509 Parameters",
            "provider": "wabengine",
            "reason": "CRL [deleted]",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "delete",
            "type": "X509 Parameters"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_account.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Account\" object=\"account_154954844398@local1@application_154954844399\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Account",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "account_154954844398@local1@application_154954844399",
            "type": "Account"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_apikey.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Apikey\" object=\"apikey_154954882800\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Apikey",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "apikey_154954882800",
            "type": "Apikey"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_application.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Application\" object=\"application_154954836612\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Application",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "application_154954836612",
            "type": "Application"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_apppath.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Apppath\" object=\"account_154954841440@local1@device_154954841439:rdp[<None>:<C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe>]\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Apppath",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "account_154954841440@local1@device_154954841439:rdp[<None>:<C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe>]",
            "type": "Apppath"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_approval.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Approval\" object=\"<Approval(uid=\\'168c849f0378d7f4005056b69255\\', status=4, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n\" user=\"user1\" client_ip=\"127.0.0.1\" infos=\"\"",
        "event": {
            "action": "Approval",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "127.0.0.1"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "<Approval(uid=\\'168c849f0378d7f4005056b69255\\', status=4, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n",
            "type": "Approval"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_authorization.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Authorization\" object=\"user_group_154954865272:target_group_154954865373\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Authorization",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "user_group_154954865272:target_group_154954865373",
            "type": "Authorization"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_checkoutpolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"CheckoutPolicy\" object=\"checkout_policy_154954874456\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CheckoutPolicy",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "checkout_policy_154954874456",
            "type": "CheckoutPolicy"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_cluster.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"delete\" type=\"Cluster\" object=\"cluster_154954875802\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Cluster",
            "provider": "wabengine",
            "type": [
                "deletion"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "delete",
            "object": "cluster_154954875802",
            "type": "Cluster"
        }
    }
    	
	```


=== "wabaudit_action_delete_type_device.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"add\" type=\"Device\" object=\"QA_DEVICE_SSH_SHELL_SESSION\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"Host [10.10.45.148], Alias [QA_DEVICE_SSH_SHELL_SESSION_ALIAS]\"",
        "event": {
            "action": "Device",
            "provider": "wabengine",
            "reason": "Host [10.10.45.148], Alias [QA_DEVICE_SSH_SHELL_SESSION_ALIAS]",
            "type": [
                "creation"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "add",
            "object": "QA_DEVICE_SSH_SHELL_SESSION",
            "type": "Device"
        }
    }
    	
	```


=== "wabaudit_action_download_type_backup_restore.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"download\" type=\"Backup/Restore\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' downloaded]\"",
        "event": {
            "action": "Backup/Restore",
            "category": [
                "database"
            ],
            "provider": "wabengine",
            "reason": "Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' downloaded]"
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "download",
            "type": "Backup/Restore"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_ConnectionPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"ConnectionPolicy\" object=\"SSH\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"methods [Add < PASSWORD_VAULT, PUBKEY_VAULT, PASSWORD_INTERACTIVE and 1 other(s) >, Remove < PUBKEY_VAULT, PASSWORD_MAPPING, PASSWORD_VAULT and 1 other(s) >], Data [session[allow_multi_channels]: 'False' => 'on']\"",
        "event": {
            "action": "ConnectionPolicy",
            "provider": "wabengine",
            "reason": "methods [Add < PASSWORD_VAULT, PUBKEY_VAULT, PASSWORD_INTERACTIVE and 1 other(s) >, Remove < PUBKEY_VAULT, PASSWORD_MAPPING, PASSWORD_VAULT and 1 other(s) >], Data [session[allow_multi_channels]: 'False' => 'on']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "edit",
            "object": "SSH",
            "type": "ConnectionPolicy"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_CredChgInfo.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"CredChgInfo\" object=\"local1/None\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CredChgInfo",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "local1/None",
            "type": "CredChgInfo"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_CredChgPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"CredChgPolicy\" object=\"password_change_policy_name_154954918865\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CredChgPolicy",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "password_change_policy_name_154954918865",
            "type": "CredChgPolicy"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Globaldomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Globaldomain\" object=\"global_domain_154954904486\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"credchgplugin ['None' to 'Windows'], credchgpolicy ['None' to 'default'], adminAccount ['None' to 'account_154954904487...']\"",
        "event": {
            "action": "Globaldomain",
            "provider": "wabengine",
            "reason": "credchgplugin ['None' to 'Windows'], credchgpolicy ['None' to 'default'], adminAccount ['None' to 'account_154954904487...']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "global_domain_154954904486",
            "type": "Globaldomain"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Ldapdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Ldapdomain\" object=\"domain_154955334798\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"description ['some description' to 'updated'], snAttribute ['' to 'updated']\"",
        "event": {
            "action": "Ldapdomain",
            "provider": "wabengine",
            "reason": "description ['some description' to 'updated'], snAttribute ['' to 'updated']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "edit",
            "object": "domain_154955334798",
            "type": "Ldapdomain"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Localdomain.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Localdomain\" object=\"local1\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"adminAccount ['None' to 'account_154954837938...']\"",
        "event": {
            "action": "Localdomain",
            "provider": "wabengine",
            "reason": "adminAccount ['None' to 'account_154954837938...']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "local1",
            "type": "Localdomain"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Notification.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Notification\" object=\"notification_154955216694\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"flag ['16' to '0']\"",
        "event": {
            "action": "Notification",
            "provider": "wabengine",
            "reason": "flag ['16' to '0']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "notification_154955216694",
            "type": "Notification"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Profil.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Profile\" object=\"profile_154954927022\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Profile",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "profile_154954927022",
            "type": "Profile"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_PwdPolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"PwdPolicy\" object=\"default\" user=\"admin\" client_ip=\"10.10.45.212\" infos=\"pwdMinLowerLetter ['1' to '0'], rsaMinLength ['4096' to '1024']\"",
        "event": {
            "action": "PwdPolicy",
            "provider": "wabengine",
            "reason": "pwdMinLowerLetter ['1' to '0'], rsaMinLength ['4096' to '1024']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "edit",
            "object": "default",
            "type": "PwdPolicy"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_RecordingOptions.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Recording Options\" user=\"admin\" client_ip=\"10.10.43.28\" infos=\"Recording Options ['No encryption, with checksum' to 'No encryption, no checksum']\"",
        "event": {
            "action": "Recording Options",
            "provider": "wabengine",
            "reason": "Recording Options ['No encryption, with checksum' to 'No encryption, no checksum']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.43.28"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.43.28",
            "ip": "10.10.43.28"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "edit",
            "type": "Recording Options"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Service.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Service\" object=\"device_154954931097:ssh\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Service",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "device_154954931097:ssh",
            "type": "Service"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Targetgroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Targetgroup\" object=\"target_group_154954945465\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"Description ['some desc' to 'some other desc']\"",
        "event": {
            "action": "Targetgroup",
            "provider": "wabengine",
            "reason": "Description ['some desc' to 'some other desc']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "target_group_154954945465",
            "type": "Targetgroup"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_TimeFrame.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"TimeFrame\" object=\"timeframe_154954954305\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "TimeFrame",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "timeframe_154954954305",
            "type": "TimeFrame"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_User.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"User\" object=\"user1\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"email ['qa-notify@wallix.com...' to 'qa-notify+1@wallix.c...']\"",
        "event": {
            "action": "User",
            "provider": "wabengine",
            "reason": "email ['qa-notify@wallix.com...' to 'qa-notify+1@wallix.c...']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "user1",
            "type": "User"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_UserAuth.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"UserAuth\" object=\"auth_LDAP_154955202505\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"description ['None' to 'updated while used b...']\"",
        "event": {
            "action": "UserAuth",
            "provider": "wabengine",
            "reason": "description ['None' to 'updated while used b...']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "auth_LDAP_154955202505",
            "type": "UserAuth"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_Usergroup.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Usergroup\" object=\"user_group_154954965326\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"Description ['some desc' to 'some other desc']\"",
        "event": {
            "action": "Usergroup",
            "provider": "wabengine",
            "reason": "Description ['some desc' to 'some other desc']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "user_group_154954965326",
            "type": "Usergroup"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_X509Parameters.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"X509 Parameters\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"CRL [file updated]\"",
        "event": {
            "action": "X509 Parameters",
            "provider": "wabengine",
            "reason": "CRL [file updated]",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "edit",
            "type": "X509 Parameters"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_action.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Account\" object=\"account_154954837938@local1@application_154954837837\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Account",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "account_154954837938@local1@application_154954837837",
            "type": "Account"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_application.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Application\" object=\"application_154954842057\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Application",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "application_154954842057",
            "type": "Application"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_approval.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Approval\" object=\"<Approval(uid=\\'168c849fa6a347bd005056b69255\\', status=1, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"status ['3' to '1']\"",
        "event": {
            "action": "Approval",
            "provider": "wabengine",
            "reason": "status ['3' to '1']",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "<Approval(uid=\\'168c849fa6a347bd005056b69255\\', status=1, begin=2019-02-07 15:08:00, end=2019-02-07 15:18:00, quorum=1)>\\n",
            "type": "Approval"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_authorization.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Authorization\" object=\"user_group_154954869778:target_group_154954869779\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Authorization",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "user_group_154954869778:target_group_154954869779",
            "type": "Authorization"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_checkoutpolicy.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"CheckoutPolicy\" object=\"checkout_policy_154954875282\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "CheckoutPolicy",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "checkout_policy_154954875282",
            "type": "CheckoutPolicy"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_cluster.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Cluster\" object=\"cluster_154954878267\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Cluster",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "cluster_154954878267",
            "type": "Cluster"
        }
    }
    	
	```


=== "wabaudit_action_edit_type_device.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"edit\" type=\"Device\" object=\"device_154954892089\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Device",
            "provider": "wabengine",
            "type": [
                "change"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "edit",
            "object": "device_154954892089",
            "type": "Device"
        }
    }
    	
	```


=== "wabaudit_action_list_type_accountactivity.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"accountactivity\" object=\"168c1c48f141e911005056b60af6\" user=\"admin\" client_ip=\"10.10.43.84\" infos=\"\"",
        "event": {
            "action": "accountactivity",
            "provider": "wabengine",
            "type": [
                "access"
            ]
        },
        "related": {
            "ip": [
                "10.10.43.84"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "10.10.43.84",
            "ip": "10.10.43.84"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "list",
            "object": "168c1c48f141e911005056b60af6",
            "type": "accountactivity"
        }
    }
    	
	```


=== "wabaudit_action_list_type_approval.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"Approval\" user=\"user1\" client_ip=\"10.10.45.212\" infos=\"\"",
        "event": {
            "action": "Approval",
            "provider": "wabengine",
            "type": [
                "access"
            ]
        },
        "related": {
            "ip": [
                "10.10.45.212"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "10.10.45.212",
            "ip": "10.10.45.212"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "list",
            "type": "Approval"
        }
    }
    	
	```


=== "wabaudit_action_list_type_sessionlog.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"sessionlog\" user=\"user1\" client_ip=\"127.0.0.1\" infos=\"Current sessions\"",
        "event": {
            "action": "sessionlog",
            "category": [
                "authentication"
            ],
            "provider": "wabengine",
            "reason": "Current sessions",
            "type": [
                "info"
            ]
        },
        "related": {
            "ip": [
                "127.0.0.1"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "list",
            "type": "sessionlog"
        }
    }
    	
	```


=== "wabaudit_action_restore_type_backup_restore.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"restore\" type=\"Backup/Restore\" user=\"admin\" client_ip=\"192.168.0.12\" infos=\"Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' restored]\"",
        "event": {
            "action": "Backup/Restore",
            "category": [
                "database"
            ],
            "provider": "wabengine",
            "reason": "Backup ['wab-6.0-cspn_2019-02-04_16-59-11.wbk' restored]"
        },
        "related": {
            "ip": [
                "192.168.0.12"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "192.168.0.12",
            "ip": "192.168.0.12"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "restore",
            "type": "Backup/Restore"
        }
    }
    	
	```


=== "wabaudit_approvals.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"Approvals\" user=\"user1\" client_ip=\"127.0.0.1\" infos=\"\"\n",
        "event": {
            "action": "Approvals",
            "provider": "wabengine",
            "type": [
                "access"
            ]
        },
        "related": {
            "ip": [
                "127.0.0.1"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "list",
            "type": "Approvals"
        }
    }
    	
	```


=== "wabauth.json"

    ```json
	
    {
        "message": "[wabauth] action=\"authentify\" user=\"admin\" client_ip=\"1.1.1.1\" status=\"success\" infos=\"diagnostic [Authentication success: identified with local(LOCAL), authentified with: API key Bastion(APIKEY).]\"",
        "event": {
            "category": [
                "authentication"
            ],
            "outcome": "success",
            "reason": "diagnostic [Authentication success: identified with local(LOCAL), authentified with: API key Bastion(APIKEY).]"
        },
        "related": {
            "ip": [
                "1.1.1.1"
            ],
            "user": [
                "admin"
            ]
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "admin"
        },
        "wallix": {
            "action": "authentify"
        }
    }
    	
	```


=== "wabengine_authentify.json"

    ```json
	
    {
        "message": "action=\"authentify\" user=\"username123\" client_ip=\"1.1.1.1\" status=\"failure\" infos=\"diagnostic [Authentication failed]",
        "event": {
            "category": [
                "authentication"
            ],
            "outcome": "failure",
            "reason": "\"diagnostic [Authentication failed]",
            "type": [
                "denied"
            ]
        },
        "related": {
            "ip": [
                "1.1.1.1"
            ],
            "user": [
                "username123"
            ]
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "username123"
        },
        "wallix": {
            "action": "authentify"
        }
    }
    	
	```


=== "wabengine_sessionlog.json"

    ```json
	
    {
        "message": "[wabaudit] action=\"list\" type=\"sessionlog\" user=\"user1\" client_ip=\"127.0.0.1\" infos=\"Closed sessions, Sessionlogs newly terminated\"\n",
        "event": {
            "action": "sessionlog",
            "category": [
                "authentication"
            ],
            "outcome": "success",
            "provider": "wabengine",
            "reason": "Closed sessions, Sessionlogs newly terminated",
            "type": [
                "end"
            ]
        },
        "related": {
            "ip": [
                "127.0.0.1"
            ],
            "user": [
                "user1"
            ]
        },
        "source": {
            "address": "127.0.0.1",
            "ip": "127.0.0.1"
        },
        "user": {
            "name": "user1"
        },
        "wallix": {
            "action": "list",
            "type": "sessionlog"
        }
    }
    	
	```


=== "wbauth_authentify.json"

    ```json
	
    {
        "message": "[wabauth] action=\"authentify\" user=\"username123\" client_ip=\"1.1.1.1\" status=\"failure\" infos=\"diagnostic [Authentication failed]",
        "event": {
            "category": [
                "authentication"
            ],
            "outcome": "failure",
            "reason": "\"diagnostic [Authentication failed]",
            "type": [
                "denied"
            ]
        },
        "related": {
            "ip": [
                "1.1.1.1"
            ],
            "user": [
                "username123"
            ]
        },
        "source": {
            "address": "1.1.1.1",
            "ip": "1.1.1.1"
        },
        "user": {
            "name": "username123"
        },
        "wallix": {
            "action": "authentify"
        }
    }
    	
	```





### Extracted Fields

The following table lists the fields that are extracted, normalized under the ECS format, analyzed and indexed by the parser. It should be noted that infered fields are not listed.

| Name | Type | Description                |
| ---- | ---- | ---------------------------|
|`destination.domain` | `keyword` | The domain name of the destination. |
|`destination.ip` | `ip` | IP address of the destination. |
|`destination.user.name` | `keyword` | Short name or login of the user. |
|`event.action` | `keyword` | The action captured by the event. |
|`event.category` | `keyword` | Event category. The second categorization field in the hierarchy. |
|`event.dataset` | `keyword` | Name of the dataset. |
|`event.end` | `date` | event.end contains the date when the event ended or when the activity was last observed. |
|`event.outcome` | `keyword` | The outcome of the event. The lowest level categorization field in the hierarchy. |
|`event.provider` | `keyword` | Source of the event. |
|`event.reason` | `keyword` | Reason why this event happened, according to the source |
|`event.start` | `date` | event.start contains the date when the event started or when the activity was first observed. |
|`event.type` | `keyword` | Event type. The third categorization field in the hierarchy. |
|`file.directory` | `keyword` | Directory where the file is located. |
|`file.extension` | `keyword` | File extension, excluding the leading dot. |
|`file.name` | `keyword` | Name of the file including the extension, without the directory. |
|`file.path` | `keyword` | Full path to the file, including the file name. |
|`host.ip` | `ip` | Host ip addresses. |
|`host.name` | `keyword` | Name of the host. |
|`process.command_line` | `wildcard` | Full command line that started the process. |
|`process.working_directory` | `keyword` | The working directory of the process. |
|`service.name` | `keyword` | Name of the service. |
|`source.domain` | `keyword` | The domain name of the source. |
|`source.ip` | `ip` | IP address of the source. |
|`source.port` | `long` | Port of the source. |
|`user.id` | `keyword` | Unique identifier of the user. |
|`user.name` | `keyword` | Short name or login of the user. |



For more information on the Intake Format, please find the code of the Parser, Smart Descriptions, and Supported Events [here](https://github.com/SEKOIA-IO/intake-formats/tree/main/Wallix/wallix-bastion).