...
| `static: Device Inventory Info: Collect` | `type_name` | OCSF type name | `string` | Always 'Device Inventory Info: Collect' for this event type |
...
| `static: Desktop` | `device.type` | OCSF device type ID | `string` | Direct mapping of type to desktop type |
...
| `hostname` | `device.hostname` | Device hostname | `string` | Direct mapping of hostname |
...
| `subnet.name` | `device.network_interfaces[0].name` | Network interface name | `string` | Direct mapping of subnet name; default to 'eth0' if missing |
...
| `calculated: subnet` | `device.network_interfaces[0].type` | Network interface type | `string` | Extracted based on subnet field |
| `calculated: subnet` | `device.network_interfaces[0].type_id` | OCSF network interface type ID | `integer` | Extracted based on subnet field |
| `firstseen` | `device.first_seen_time` | Device first seen timestamp | `timestamp` | Convert ISO 8601 to Unix epoch |
| `lastseen` | `device.last_seen_time` | Device last seen timestamp | `timestamp` | Convert ISO 8601 to Unix epoch |
| `machine_boottime` | `device.boot_time` | Device boot time | `timestamp` | Convert ISO 8601 to Unix epoch |
| `installdate` | `device.created_time` | Device creation timestamp | `timestamp` | Convert ISO 8601 to Unix epoch |
| `description` | `device.desc` | Device description | `string` | Direct mapping to description field |
| `policy` | `device.is_managed` | Is device managed | `boolean` | Depends on policy existance |
...
| `producttype` | `device.model` | Model of the device | `string` | Direct mapping of device model |
